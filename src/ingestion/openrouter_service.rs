    ...
    fn extract_json_from_response(&self, response_text: &str) -> IngestionResult<String> {
        ...
        for value in deserialize_stream {
            match value {
                ...
                Err(_) => continue, // Keep looking if parsing fails (though typically it stops at first error)
            }
        }

        // Fallback: simpler extraction if stream parsing failed (e.g. malformed JSON)
        // This handles cases where the stream parser might have been too strict or confused
        ...
        // If all else fails, return trimmed text and let the caller try to parse it (and fail with details)
...
That should work."###;
    ...
    fn test_extract_json_with_trailing_brace() {
        ...
        // The old logic would include everything up to the last '}'
