...
use crate::transform::iterator_stack::execution_engine::ExecutionResult;
use crate::transform::iterator_stack::field_alignment::AlignmentValidationResult;
use crate::transform::shared_utilities::{
    collect_expressions_from_schema_with_keys, execute_chains_with_engine, parse_expressions_batch,
};
...
pub fn execute_multi_chain_coordination_with_monitoring(
    ...
    info!(
        "⏱️ Enhanced multi-chain coordination completed in {:?}",
        total_duration
    );
...
fn parse_expressions_with_monitoring(
    ...
    info!(
        "🔗 Starting batch parsing of {} expressions",
        expressions.len()
    );
    ...
    info!(
        "📊 Parsing completed: {}/{} expressions parsed successfully ({:.1}% success rate)",
        parsed_chains.len(),
        expressions.len(),
        success_rate
    );
...
fn execute_multi_chain_with_engine_enhanced(
    ...
    let execution_result = execute_chains_with_engine(parsed_chains, alignment_result, input_data)?;
    ...
    let total_duration = start_time.elapsed();
    info!(
        "⏱️ Enhanced multi-chain execution completed in {:?}",
        total_duration
    );
...
fn convert_input_values_to_json(
    input_values: &HashMap<String, JsonValue>,
) -> Result<JsonValue, SchemaError> {
