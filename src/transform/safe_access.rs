...
use crate::atom::{Atom, Molecule, MoleculeBehavior, MoleculeRange};
...
pub trait TransformSafeDataAccess {
    ...
    fn get_readonly_atom(
        &self,
        atom_uuid: &str,
    ) -> Result<ReadOnlyAtom, crate::schema::SchemaError>;

    /// Get read-only access to a molecule.
    fn get_readonly_molecule(
        &self,
        molecule_uuid: &str,
    ) -> Result<ReadOnlyMolecule, crate::schema::SchemaError>;

    /// Get read-only access to a molecule range.
    fn get_readonly_molecule_range(
        &self,
        molecule_range_uuid: &str,
    ) -> Result<ReadOnlyMoleculeRange, crate::schema::SchemaError>;
}
...
impl TransformSafeDataAccess for DatabaseTransformDataAccess {
    fn get_readonly_atom(
        ...
        let atom = self
            ...
            .ok_or_else(|| {
                crate::schema::SchemaError::InvalidField(format!("Atom '{}' not found", atom_uuid))
            })?;
    ...
    fn get_readonly_molecule_range(
        ...
        let molecule_range = self
            .db_ops
            .get_item::<MoleculeRange>(&format!("ref:{}", molecule_range_uuid))?
            .ok_or_else(|| {
                crate::schema::SchemaError::InvalidField(format!(
                    "MoleculeRange '{}' not found",
                    molecule_range_uuid
                ))
            })?;
