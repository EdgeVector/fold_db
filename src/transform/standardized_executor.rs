...
/// Orchestrated transform executor that integrates with the event orchestrator
pub struct OrchestratedTransformExecutor {
    executor: StandardizedTransformExecutor,
    orchestrator: Arc<TransformOrchestrator>,
    message_bus: Arc<MessageBus>,
}

impl OrchestratedTransformExecutor {
    /// Create a new orchestrated transform executor
    pub fn new(
        message_bus: Arc<MessageBus>,
        orchestrator: Arc<TransformOrchestrator>,
    ) -> Self {
    ...
    /// Execute a transform with full event orchestration
    pub fn execute_transform_orchestrated<P>(
        &self,
        transform: &Transform,
        input_provider: &P,
        mutation_executor: &dyn MutationExecutor,
    ) -> Result<StandardizedExecutionResult, SchemaError>
    where
        P: InputProvider,
    {
        ...
        info!("🎼 Starting orchestrated transform execution for transform: {}", transform.get_output());
        
        // Phase 1: Gather inputs (event-driven or direct)
        let (input_values, input_gathering_duration) = self.gather_inputs_orchestrated(transform, input_provider)?;
        ...
        // Phase 4: Execute mutations and publish events
        if !mutations.is_empty() {
            info!("📝 Executing {} mutations for transform results", mutations.len());
            mutation_executor.execute_mutations(&mutations)?;
            
            // Publish FieldValueSet events for each mutation
            self.publish_mutation_events(&mutations)?;
            
            info!("✅ All mutations executed and events published");
        } else {
            warn!("⚠️ No mutations prepared for transform results");
        }
        
        // Phase 5: Publish TransformExecuted event
        self.publish_transform_executed_event(transform, &computation_result)?;
        
        let total_duration = start_time.elapsed();
        info!("🎯 Orchestrated transform execution completed in {:?}", total_duration);
    ...
    /// Phase 1: Gather inputs with event orchestration
    fn gather_inputs_orchestrated<P>(
        &self,
        transform: &Transform,
        input_provider: &P,
    ) -> Result<(HashMap<String, JsonValue>, std::time::Duration), SchemaError>
        ...
        P: InputProvider,
    {
        ...
        info!("📥 Phase 1: Gathering inputs with orchestration for transform '{}'", transform.get_output());
        ...
        info!("🔍 Processing {} inputs with orchestration: {:?}", inputs_to_process.len(), inputs_to_process);
        
        for input_name in inputs_to_process {
            match input_provider.get_input(&input_name) {
                Ok(value) => {
                    input_values.insert(input_name.clone(), value);
                    info!("✅ Gathered input '{}' with orchestration", input_name);
                }
                Err(e) => {
        ...
        let duration = start_time.elapsed();
        info!("✅ Phase 1 completed: Gathered {} inputs with orchestration in {:?}", input_values.len(), duration);
        
        Ok((input_values, duration))
    }

    /// Phase 2: Run transform computation (delegate to base executor)
    fn run_transform_computation(
        &self,
        transform: &Transform,
        input_values: &HashMap<String, JsonValue>,
    ) -> Result<(JsonValue, std::time::Duration), SchemaError> {
        self.executor.run_transform_computation(transform, input_values)
    }

    /// Phase 3: Prepare mutations (delegate to base executor)
    fn prepare_mutations(
        &self,
        transform: &Transform,
        computation_result: &JsonValue,
    ) -> Result<(Vec<Mutation>, std::time::Duration), SchemaError> {
        self.executor.prepare_mutations(transform, computation_result)
    }

    /// Publish FieldValueSet events for mutations
    fn publish_mutation_events(&self, mutations: &[Mutation]) -> Result<(), SchemaError> {
        for mutation in mutations {
            // Parse the mutation to extract field information
            for (field_name, value) in &mutation.fields_and_values {
                let field_path = format!("{}.{}", mutation.schema_name, field_name);
                
                let field_value_event = FieldValueSet::new(
                    field_path,
                    value.clone(),
                    "orchestrated_transform_executor",
                );
                
                match self.message_bus.publish(field_value_event) {
                    Ok(_) => {
                        info!("📡 Published FieldValueSet event for {}.{}", mutation.schema_name, field_name);
                    }
                    Err(e) => {
                        error!("❌ Failed to publish FieldValueSet event for {}.{}: {}", mutation.schema_name, field_name, e);
                        return Err(SchemaError::InvalidData(format!(
                            "Failed to publish FieldValueSet event: {}", e
                        )));
                    }
                }
            }
    ...
    fn publish_transform_executed_event(
        &self,
        transform: &Transform,
        computation_result: &JsonValue,
    ) -> Result<(), SchemaError> {
        let transform_id = format!("transform_{}", transform.get_output().replace('.', "_"));
        let result_str = serde_json::to_string(computation_result)
            .unwrap_or_else(|_| "{}".to_string());
        
        let executed_event = TransformExecuted::new(transform_id, result_str);
        
        match self.message_bus.publish(executed_event) {
            Ok(_) => {
                info!("📡 Published TransformExecuted event for transform: {}", transform.get_output());
                Ok(())
            }
            Err(e) => {
                error!("❌ Failed to publish TransformExecuted event: {}", e);
                Err(SchemaError::InvalidData(format!(
                    "Failed to publish TransformExecuted event: {}", e
                )))
            }
        }
    }

    /// Queue a transform for orchestrated execution
    pub fn queue_transform_for_execution(
        &self,
        transform: &Transform,
        mutation_hash: &str,
    ) -> Result<(), SchemaError> {
        // Parse the output field to extract schema and field
        let output_field = transform.get_output();
        if let Some(dot_pos) = output_field.find('.') {
            let schema_name = &output_field[..dot_pos];
            let field_name = &output_field[dot_pos + 1..];
            
            // Add task to the orchestrator queue
            self.orchestrator.add_task(schema_name, field_name, mutation_hash)?;
...
impl MutationExecutor for MutationServiceExecutor {
    fn execute_mutation(&self, mutation: &Mutation) -> Result<(), SchemaError> {
        // Get schema for validation
        // Note: This is a simplified implementation - in practice you'd need access to schema manager
        info!("🔄 Executing mutation for schema '{}'", mutation.schema_name);
        
        // For now, we'll use a placeholder implementation
        // In practice, this would use the mutation service to update field values
        warn!("⚠️ Mutation execution not fully implemented - using placeholder");
