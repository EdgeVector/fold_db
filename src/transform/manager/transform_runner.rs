...
impl TransformRunner for super::TransformManager {
    ...
    fn execute_transform_with_context(
        ...
        let transform = transforms.get(transform_id).cloned().ok_or_else(|| {
        ...
        // Handle both runtime and non-runtime contexts via run_async helper
        let input_values = Self::run_async(InputFetcher::fetch_input_values_with_context(
            &transform,
            &self.db_ops,
            mutation_context,
        ))?;
        ...
        let schema = Self::run_async(self.db_ops.get_schema(transform.get_schema_name()))?
            .ok_or_else(|| {
