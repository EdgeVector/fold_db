...
const BackfillMonitor = () => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'InProgress': return 'text-blue-700 bg-blue-50'
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        ...
        {displayedBackfills.length === 0 ? (
          ...
          <div className="space-y-3">
            {displayedBackfills.map((backfill, index) => (
              <div key={`${backfill.transform_id}-${index}`} className={`p-3 rounded-lg border ${getStatusColor(backfill.status)}`}>
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    <span className="text-xl">{getStatusIcon(backfill.status)}</span>
                    <div>
