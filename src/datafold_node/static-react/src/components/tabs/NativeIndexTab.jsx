...
export default function NativeIndexTab({ onResult }) {
  ...
  const schemasWithFields = useMemo(() => {
    return (approvedSchemas || []).map(s => ({
      name: s.name,
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        <div className="mb-2 flex items-center justify-between">
          <h4 className="text-md font-medium text-gray-900">Approved Schemas and Fields</h4>
          {schemasLoading && <span className="text-xs text-gray-500">Loading schemas...</span>}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {schemasWithFields.map(({ name, fields }) => (
            <div key={name} className="border rounded p-2 bg-gray-50">
              <div className="text-sm font-semibold text-gray-800">{name}</div>
              <div className="mt-1 flex flex-wrap gap-1">
                {fields.map(f => (
                  <span key={f} className="text-xs bg-white border px-2 py-0.5 rounded">{f}</span>
                ))}
              </div>
            </div>
          ))}
          {schemasWithFields.length === 0 && (
            <div className="text-xs text-gray-500">No approved schemas.</div>
          )}
        </div>
      </div>
