...
fn rule_tree(pool: &Arc<SledPool>) -> Result<sled::Tree, FoldDbError> {
    ...
    guard.db().open_tree(SHARE_RULE_TREE)
...
fn sub_tree(pool: &Arc<SledPool>) -> Result<sled::Tree, FoldDbError> {
    let guard = pool.acquire_arc()
        .map_err(|e| FoldDbError::Database(format!("Failed to acquire SledPool: {}", e)))?;
    guard.db().open_tree(SHARE_SUB_TREE)
        .map_err(|e| FoldDbError::Database(format!("Failed to open share_subscriptions tree: {}", e)))
}

pub fn create_share_rule(
    pool: &Arc<SledPool>,
    rule: ShareRule,
) -> Result<(), FoldDbError> {
...
pub fn list_share_rules(
    pool: &Arc<SledPool>,
) -> Result<Vec<ShareRule>, FoldDbError> {
    let tree = rule_tree(pool)?;
    let mut rules = Vec::new();
    for entry in tree.iter() {
        let (_, value) = entry
            .map_err(|e| FoldDbError::Database(format!("Failed to iterate share rules tree: {}", e)))?;
...
pub fn deactivate_share_rule(
    ...
    if let Some(value) = tree.get(key.as_bytes()).map_err(|e| FoldDbError::Database(format!("Failed to read share rules: {}", e)))? {
    ...
    Err(FoldDbError::Database(format!("Share rule {} not found", rule_id)))
}
...
pub fn list_share_subscriptions(
    ...
    for entry in tree.iter() {
        let (_, value) = entry
            .map_err(|e| FoldDbError::Database(format!("Failed to iterate share subscriptions tree: {}", e)))?;
...
pub fn get_share_subscription(
    ...
    if let Some(value) = tree.get(key.as_bytes()).map_err(|e| FoldDbError::Database(format!("Failed to read share subscriptions: {}", e)))? {
        let sub: ShareSubscription = serde_json::from_slice(&value)?;
        return Ok(Some(sub));
    }
    Ok(None)
