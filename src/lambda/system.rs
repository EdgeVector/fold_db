...
impl LambdaContext {
    ...
    pub async fn query_logs(
        ...
        let ctx = Self::get()?;
        ctx.logger
            .query(user_id, limit, from_timestamp)
            .await
    ...
    pub async fn reset_database(confirm: bool, user_id: String) -> Result<(), IngestionError> {
        ...
        if !confirm {
            return Err(IngestionError::InvalidInput(
                "Reset confirmation required".to_string(),
            ));
        }
        ...
        processor
            .perform_database_reset(Some(&user_id))
            .await
    ...
    pub async fn reset_schema_service(
        ...
        schema_client.reset_schema_service().await.map_err(|e| {
            IngestionError::InvalidInput(format!("Schema service reset failed: {}", e))
        })?;
    ...
    pub async fn test_logger(user_id: &str) -> Result<Value, IngestionError> {
        ...
        // Test 1: INFO level
        logger
            .info("test_info", "Testing INFO level logging")
            .await
        ...
        // Test 2: ERROR level
        logger
            .error("test_error", "Testing ERROR level logging")
            .await
        ...
        // Test 3: WARN level
        logger
            .warn("test_warn", "Testing WARN level logging")
            .await
        ...
        // Test 4: DEBUG level
        logger
            .debug("test_debug", "Testing DEBUG level logging")
            .await
        ...
        // Test 5: TRACE level
        logger
            .trace("test_trace", "Testing TRACE level logging")
            .await
        ...
        logger
            .info("workflow_started", "Beginning test workflow")
            .await
            .map_err(|e| IngestionError::InvalidInput(format!("Workflow test failed: {}", e)))?;
        logger
            .debug("workflow_step_1", "Processing step 1")
            .await
            .map_err(|e| IngestionError::InvalidInput(format!("Workflow test failed: {}", e)))?;
        logger
            .debug("workflow_step_2", "Processing step 2")
            .await
            .map_err(|e| IngestionError::InvalidInput(format!("Workflow test failed: {}", e)))?;
        logger
            .debug("workflow_step_3", "Processing step 3")
            .await
            .map_err(|e| IngestionError::InvalidInput(format!("Workflow test failed: {}", e)))?;
        logger
            .info("workflow_completed", "Workflow completed successfully")
            .await
            .map_err(|e| IngestionError::InvalidInput(format!("Workflow test failed: {}", e)))?;
    ...
    pub async fn update_database_config(
        ...
        processor
            .update_database_configuration(new_config)
            .await
            .map_err(|e| {
                IngestionError::InvalidInput(format!("Failed to update configuration: {}", e))
            })?;
