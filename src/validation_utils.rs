...
impl ValidationUtils {
    ...
    pub fn require_non_empty_collection<T>(collection: &[T], field_name: &str) -> Result<(), SchemaError> {
    ...
    pub fn require_some<'a, T>(option: &'a Option<T>, field_name: &str) -> Result<&'a T, SchemaError> {
        option.as_ref().ok_or_else(|| SchemaError::InvalidField(
    ...
    pub fn require_valid_api_key(api_key: &str, service_name: &str) -> Result<(), SchemaError> {
        ...
        if api_key.len() < 10 {
            return Err(SchemaError::InvalidField(
                format!("{} API key appears to be too short", service_name)
            ));
        }
