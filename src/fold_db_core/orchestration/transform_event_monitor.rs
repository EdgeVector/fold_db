...
impl TransformEventMonitor {
    ...
    /// Stop monitoring (the thread will be stopped when the TransformEventMonitor is dropped)
    pub fn stop_monitoring(&mut self) {
        if let Some(_handle) = self._monitoring_thread.take() {
            ...
            info!("TransformEventMonitor: Stopping unified event monitoring");
...
impl Drop for TransformEventMonitor {
    fn drop(&mut self) {
