use crate::schema::types::{SchemaError, Mutation, MutationType};
...
impl HashRangeProcessor {
    ...
    pub fn store_hashrange_transform_result_with_message_bus(
        ...
        debug!("🔍 Processing {} data entries with {} fields", transform_data.data_entries.len(), field_names.len());
    ...
    fn get_hashrange_schema(
        ...
        if !matches!(schema.schema_type, crate::schema::types::SchemaType::HashRange) {
            return Err(SchemaError::InvalidData(format!("Schema '{}' is not a HashRange schema", schema_name)));
        }
    ...
    fn parse_transform_result(
        ...
        let result_obj = result.as_object()
            .ok_or_else(|| SchemaError::InvalidData(format!("HashRange transform result must be an object, got: {}", result)))?;

        debug!("🔍 HashRange transform result structure: {} keys", result_obj.len());
        ...
        debug!("🔍 HashRange result: {} hash keys, {} range keys", hash_keys.len(), range_keys.len());
        ...
        // This will be handled in process_hashrange_data
        debug!("🔍 Field arrays: {:?}", field_arrays.keys().collect::<Vec<_>>());
    ...
    fn find_content_field(field_names: &[String]) -> String {
        ...
        field_names.iter()
            .find(|name| {
                ...
                content_indicators.iter().any(|indicator| lower_name.contains(indicator))
            })
            .cloned()
    ...
    fn extract_field_values_for_entry(
        ...
        for field_name in field_names {
            let field_value = transform_data.field_arrays.get(field_name)
    ...
    fn submit_mutation_through_message_bus(
        ...
        for (field_name, value) in &mutation.fields_and_values {
            if field_name != "hash_key" && field_name != "range_key" {
                debug!("🔧 Submitting field '{}' for word '{}' through message bus", field_name, hash_key);
                ...
                message_bus.publish(field_value_request)
                    .map_err(|e| SchemaError::InvalidData(format!("Failed to submit field value for {}.{}: {}", schema_name, field_name, e)))?;
                
                debug!("✅ HashRange field value submitted successfully for {}.{} with word '{}'", schema_name, field_name, hash_key);
            }
        }
