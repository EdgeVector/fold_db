...
impl SchemaDataFetcher {
    ...
    pub fn fetch_schema_data_for_range_key(
        ...
        println!("🎯 TransformManager: Fetching data for range_key '{}' in schema '{}'", range_key, schema_name);
        ...
        for field_name in field_names {
            Self::process_field_for_range_key(db_ops, schema_name, &field_name, range_key, &mut record)?;
        }
        ...
        println!("✅ TransformManager: Returning incremental data for range_key '{}': {}", range_key, result);
        Ok(result)
    }
    ...
    pub fn fetch_schema_data_for_hashrange_key(
        ...
        for field_name in field_names {
            Self::process_field_for_hashrange_key(db_ops, schema_name, &field_name, hash_key, range_key, &mut record)?;
        }
    ...
    pub fn fetch_range_schema_data(
        ...
        let schema_array = Self::convert_range_records_to_array(schema, schema_name, records_by_range_key)?;
    ...
    pub fn fetch_hashrange_schema_data(
        ...
        info!("🔍 TransformManager: Processing HashRange schema '{}'", schema_name);
        ...
        let mut records_by_keys: std::collections::HashMap<(String, String), serde_json::Map<String, serde_json::Value>> = std::collections::HashMap::new();
        ...
        info!("🔍 TransformManager: Found {} records for HashRange schema '{}'", schema_array.len(), schema_name);
        Ok(schema_array)
    }
    ...
    pub fn fetch_simple_schema_data(
        ...
        info!("🔍 TransformManager: Processing simple schema '{}'", schema_name);
        ...
        info!("🔍 TransformManager: Found {} records for simple schema '{}'", schema_array.len(), schema_name);
        Ok(schema_array)
    }
    ...
    fn process_field_for_range_key(
        ...
        println!("🔍 TransformManager: Looking for Range molecule for field '{}' with range_key '{}'", field_name, range_key);
        ...
        println!("🔍 TransformManager: Looking for molecule: {}", molecule_key);
        ...
        match db_ops.get_item::<crate::atom::MoleculeRange>(&molecule_key) {
            ...
            Ok(None) => {
                println!("⚠️ MoleculeRange '{}' not found for field '{}'", molecule_key, field_name);
            }
            Err(e) => {
                println!("❌ Error loading MoleculeRange '{}' for field '{}': {}", molecule_key, field_name, e);
            }
        }
    ...
    fn process_field_for_hashrange_key(
        ...
        let molecule_key = format!("ref:{}_{}_hashrange_{}_{}", schema_name, field_name, hash_key, range_key);
        println!("🔍 TransformManager: Looking for molecule: {}", molecule_key);
        ...
        match db_ops.get_item::<crate::atom::MoleculeHashRange>(&molecule_key) {
            Ok(Some(hashrange_molecule)) => {
                ...
                if hashrange_molecule.hash_value == hash_key && hashrange_molecule.range_value == range_key {
            ...
            Ok(None) => {
                println!("⚠️ MoleculeHashRange '{}' not found for field '{}'", molecule_key, field_name);
            }
            Err(e) => {
                println!("❌ Error loading MoleculeHashRange '{}' for field '{}': {}", molecule_key, field_name, e);
            }
        }
        ...
        match db_ops.get_item::<crate::atom::Atom>(&format!("atom:{}", atom_uuid)) {
            Ok(Some(atom)) => {
                ...
                if let Some(value) = content.get("value") {
                    ...
                    println!("🔍 TransformManager: Added field '{}' = {} to record", field_name, value);
                }
            }
            Ok(None) => {
    ...
    fn process_range_field(
        ...
        schema_name: &str,
        field_name: &str,
        records_by_range_key: &mut std::collections::HashMap<String, serde_json::Map<String, serde_json::Value>>,
    ) -> Result<(), SchemaError> {
        ...
        for key_str in range_keys {
            Self::process_range_molecule(db_ops, schema_name, field_name, &key_str, records_by_range_key, &mut found_data)?;
        }
    ...
    fn try_legacy_range_format(
        ...
        match db_ops.get_item::<crate::atom::Molecule>(&format!("{}{}", MOLECULE_PREFIX, single_molecule_uuid)) {
                ...
                match db_ops.get_item::<crate::atom::Atom>(&format!("{}{}", ATOM_PREFIX, atom_uuid)) {
                    Ok(Some(atom)) => {
                        ...
                        if let Some(value) = content.get("value") {
                            ...
                            let record = records_by_range_key.entry(DEFAULT_RANGE_KEY.to_string()).or_default();
                            record.insert(field_name.to_string(), value.clone());
    ...
    fn process_hashrange_field(
        ...
        schema_name: &str,
        field_name: &str,
        records_by_keys: &mut std::collections::HashMap<(String, String), serde_json::Map<String, serde_json::Value>>,
    ) -> Result<(), SchemaError> {
        println!("🔍 TransformManager: Looking for HashRange molecules for field '{}'", field_name);
        
        // Look for all HashRange molecules that match the pattern: {schema_name}_{field_name}_hashrange_{hash_key}_{range_key}
        // Use prefix scanning for better performance
        let hashrange_prefix = format!("{}{}_{}_hashrange_", MOLECULE_PREFIX, schema_name, field_name);
        let hashrange_keys = db_ops.list_items_with_prefix(&hashrange_prefix)?;
        
        for key_str in hashrange_keys {
            Self::process_hashrange_molecule(db_ops, schema_name, field_name, &key_str, records_by_keys)?;
        }
        ...
        if let Some(remaining) = molecule_uuid.strip_prefix(&prefix) {
            ...
            if let Some(last_underscore_pos) = remaining.rfind('_') {
                ...
                match db_ops.get_item::<crate::atom::MoleculeHashRange>(key_str) {
                    ...
                    Ok(None) => {
                        println!("⚠️ MoleculeHashRange '{}' not found for field '{}'", key_str, field_name);
                    }
                    Err(e) => {
                        println!("❌ Error loading MoleculeHashRange '{}' for field '{}': {}", key_str, field_name, e);
                    }
                }
            }
        }
    ...
    fn convert_range_records_to_array(
        schema: &Schema,
        schema_name: &str,
        records_by_range_key: std::collections::HashMap<String, serde_json::Map<String, serde_json::Value>>,
    ) -> Result<Vec<JsonValue>, SchemaError> {
    ...
    fn convert_hashrange_records_to_array(
        schema_name: &str,
        records_by_keys: std::collections::HashMap<(String, String), serde_json::Map<String, serde_json::Value>>,
    ) -> Result<Vec<JsonValue>, SchemaError> {
    ...
    fn format_simple_schema_data(
        ...
        if has_range_structure {
            ...
            let range_keys: Vec<String> = schema_data.values()
