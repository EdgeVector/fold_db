...
use crate::fold_db_core::mutation_completion_handler::{
    MutationCompletionError, MutationCompletionHandler,
};
...
impl MutationExecutor {
    ...
    pub fn new(
        ...
        let mutation_processor = MutationProcessor::new(Arc::clone(&schema_manager));
    ...
    pub fn write_schema(&mut self, mutation: Mutation) -> Result<String, SchemaError> {
        ...
        log_feature!(
            LogFeature::Mutation,
            info,
            "Generated mutation ID {} for completion tracking",
            mutation_id
        );
        ...
        log_feature!(
            LogFeature::Mutation,
            info,
            "Pre-registered mutation {} with completion handler",
            mutation_id
        );

        // 1. Prepare mutation and validate schema
        let (schema, processed_mutation, mutation_hash) = self
            .mutation_processor
            .prepare_mutation_and_schema(mutation)?;

        // 2. Create mutation service and delegate field updates
        let mutation_service = MutationService::new(Arc::clone(&self.message_bus));
        let result = self.mutation_processor.process_field_mutations_via_service(
            &mutation_service,
            &schema,
            &processed_mutation,
            &mutation_hash,
        ...
        if processed_mutation.synchronous.unwrap_or(false) {
            ...
            log_feature!(
                LogFeature::Mutation,
                info,
                "Synchronous mode: signaling completion immediately for mutation ID {}",
                mutation_id
            );
            if let Err(e) = self.completion_handler.signal_completion_sync(&mutation_id) {
                warn!(
                    "Failed to signal synchronous completion for mutation {}: {}",
                    mutation_id, e
                );
            }
        } else {
            ...
            tokio::spawn(async move {
                ...
                if let Err(e) = db_ops.db().flush() {
                    log_feature!(
                        LogFeature::Mutation,
                        warn,
                        "Failed to flush database before signaling completion: {}",
                        e
                    );
                } else {
                    log_feature!(
                        LogFeature::Mutation,
                        info,
                        "Database flushed before signaling mutation completion"
                    );
                }
                ...
                log_feature!(
                    LogFeature::Mutation,
                    info,
                    "✅ Mutation {} completion signaled after database persistence",
                    mutation_id_clone
                );
            });
        }
    ...
    pub async fn wait_for_mutation(&self, mutation_id: &str) -> Result<(), SchemaError> {
        log_feature!(
            LogFeature::Mutation,
            info,
            "Waiting for completion of mutation: {}",
            mutation_id
        );
        ...
        match self
            .completion_handler
            .wait_for_completion(mutation_id)
            .await
        {
            Ok(()) => {
                log_feature!(
                ...
                let schema_error = match mutation_error {
                    ...
                    MutationCompletionError::MutationNotFound(id) => {
                        log_feature!(
                            LogFeature::Mutation,
                            warn,
                            "Mutation {} not found in completion tracking system",
                            id
                        );
                    ...
                    MutationCompletionError::SignalFailed(id, reason) => {
                        log_feature!(
                            LogFeature::Mutation,
                            error,
                            "Failed to receive completion signal for mutation {}: {}",
                            id,
                            reason
                        );
                        SchemaError::InvalidData(format!(
                    ...
                    MutationCompletionError::LockFailed(reason) => {
                        log_feature!(
                            LogFeature::Mutation,
                            error,
                            "Failed to acquire lock for mutation completion tracking: {}",
                            reason
                        );
                        SchemaError::InvalidData(format!(
