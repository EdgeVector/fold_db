...
| TRANSFORM-005 | Iterator stack execution must resolve schema data within the `input` wrapper so expressions prefixed with `input.` can access nested fields during transform coordination. | transform/iterator_stack | 2025-09-30 13:45:00 | None |
...
| SCHEMA-KEY-004 | Universal key configuration integration is enforced across query, mutation, transform, and aggregation flows. HashRange processors, mutation services, transform tests, field processing utilities, and aggregation pipelines must use shared helpers (`extract_unified_keys`, `shape_unified_result`) while preserving compatibility fields (`hash_key`, `range_key`) for legacy consumers. Aggregation compliance and documentation are tracked in SKC-7 delivery notes. | query/, mutation/, transform/, tests/, docs/ | 2025-09-22 06:10:00 | None |
