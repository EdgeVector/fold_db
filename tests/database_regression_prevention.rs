...
use datafold::schema::types::{Field, FieldVariant, Schema, SchemaType, SingleField};
...
fn test_database_consistency_validation() {
    ...
    for (field_name, field) in &loaded_schema.fields {
        ...
        let molecule_exists = fixture
            .db_ops
            .get_item::<datafold::atom::Molecule>(&format!("ref:{}", molecule_uuid))
            .expect("Failed to check molecule existence")
            .is_some();
...
fn test_schema_approval_workflow_integrity() {
    ...
    for (field_name, field) in &loaded_schema1.fields {
        assert!(
            field.molecule_uuid().is_some(),
            "Schema1 field '{}' missing molecule_uuid after approval workflow",
            field_name
        );
    }
    ...
    for (field_name, field) in &loaded_schema2.fields {
        assert!(
            field.molecule_uuid().is_some(),
            "Schema2 field '{}' missing molecule_uuid after approval workflow",
            field_name
        );
    }
...
fn test_cross_database_instance_persistence() {
    ...
    // First instance: create and approve schema
    {
        ...
        schema_core
            ...
            .expect("Failed to load schema");
        schema_core
            .approve_schema("PersistenceTestSchema")
            .expect("Failed to approve schema");

        // Verify field assignment was created
        let loaded_schema = db_ops
            .get_schema("PersistenceTestSchema")
            .expect("Failed to get schema")
            .expect("Schema not found");
...
fn test_critical_bug_fixes_integration() {
    ...
    fixture
        .schema_core
        .approve_schema("CriticalBugTest")
        .expect("Failed to approve schema");

    // Verify field assignments are created properly
    let loaded_schema = fixture
        .db_ops
        .get_schema("CriticalBugTest")
        .expect("Failed to get schema")
        .expect("Schema not found");
    ...
    fixture
        .schema_core
        .load_schema_internal(multi_field_schema)
        .expect("Failed to load multi-field schema");
    fixture
        .schema_core
        .approve_schema("MultiFieldTest")
        .expect("Failed to approve multi-field schema");

    let multi_loaded_schema = fixture
        .db_ops
        .get_schema("MultiFieldTest")
    ...
    for (field_name, field) in &multi_loaded_schema.fields {
        assert!(
            field.molecule_uuid().is_some(),
            "BUG REGRESSION: Multi-field schema field '{}' missing assignment",
            field_name
        );
    }
