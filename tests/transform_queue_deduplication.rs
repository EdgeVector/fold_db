...
async fn test_transform_queue_deduplication_by_mutation_id() {
    ...
    let result1 = queue_manager.add_item(transform_id, mutation_id)
        .expect("Failed to add first item");
    assert!(result1, "First addition should return true (item was added)");
    
    // Second addition with same transform_id and mutation_id should be deduplicated
    let result2 = queue_manager.add_item(transform_id, mutation_id)
        .expect("Failed to attempt second addition");
    assert!(!result2, "Second addition should return false (item was deduplicated)");
    
    // Third addition should also be deduplicated
    let result3 = queue_manager.add_item(transform_id, mutation_id)
        .expect("Failed to attempt third addition");
    assert!(!result3, "Third addition should return false (item was deduplicated)");
    
    // Verify only one item is in the queue
    let queued_transforms = queue_manager.list_queued_transforms()
        .expect("Failed to list queued transforms");
    assert_eq!(queued_transforms.len(), 1, "Should have exactly 1 item in queue");
    assert_eq!(queued_transforms[0], transform_id, "Queued transform should match");
    ...
    let result4 = queue_manager.add_item(transform_id, mutation_id_2)
        .expect("Failed to add item with different mutation_id");
    assert!(result4, "Addition with different mutation_id should return true");
    
    let result5 = queue_manager.add_item(transform_id, mutation_id_3)
        .expect("Failed to add item with third mutation_id");
    assert!(result5, "Addition with third mutation_id should return true");
    
    // Verify we now have 3 items in the queue (same transform, different mutations)
    let queued_transforms_after = queue_manager.list_queued_transforms()
        .expect("Failed to list queued transforms after multiple additions");
    assert_eq!(queued_transforms_after.len(), 3, "Should have exactly 3 items in queue");
    
    // All items should be the same transform_id
    for transform in &queued_transforms_after {
        assert_eq!(transform, transform_id, "All queued items should have same transform_id");
    }
    ...
    let result6 = queue_manager.add_item(transform_id_2, shared_mutation_id)
    ...
    let result7 = queue_manager.add_item(transform_id_3, shared_mutation_id)
    ...
    let final_queued_transforms = queue_manager.list_queued_transforms()
        .expect("Failed to list final queued transforms");
    assert_eq!(final_queued_transforms.len(), 5, "Should have exactly 5 items in queue");
    ...
    let duplicate_result1 = queue_manager.add_item(transform_id, mutation_id)
        .expect("Failed to attempt duplicate addition");
    assert!(!duplicate_result1, "Duplicate addition should still be deduplicated");
    
    let duplicate_result2 = queue_manager.add_item(transform_id_2, shared_mutation_id)
        .expect("Failed to attempt duplicate addition");
    assert!(!duplicate_result2, "Duplicate addition should still be deduplicated");
    
    // Queue size should remain the same
    let final_size = queue_manager.list_queued_transforms()
        .expect("Failed to list queued transforms for final check");
    assert_eq!(final_size.len(), 5, "Queue size should remain unchanged after duplicate attempts");
...
fn test_transform_queue_deduplication_empty_state() {
    ...
    let queued_transforms = queue_manager.list_queued_transforms()
        .expect("Failed to list queued transforms");
    assert_eq!(queued_transforms.len(), 1, "Empty queue should have 1 item after addition");
    
    // Duplicate addition should be deduplicated
    let duplicate_result = queue_manager.add_item(transform_id, mutation_id)
        .expect("Failed to attempt duplicate addition");
    ...
    let final_queued_transforms = queue_manager.list_queued_transforms()
        .expect("Failed to list final queued transforms");
    assert_eq!(final_queued_transforms.len(), 1, "Queue size should remain 1 after duplicate");
...
fn test_transform_queue_key_generation() {
    ...
    let result3 = queue_manager.add_item(transform_id, different_mutation)
    ...
    let result4 = queue_manager.add_item(different_transform, mutation_id)
...
fn test_transform_queue_deduplication_concurrent_simulation() {
    ...
    let queued_transforms = queue_manager.list_queued_transforms()
        .expect("Failed to list queued transforms");
    assert_eq!(queued_transforms.len(), 1, "Should have exactly 1 item despite multiple additions");
    assert_eq!(queued_transforms[0], transform_id, "Queued transform should match");
