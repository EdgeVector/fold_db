...
use datafold::transform::shared_utilities::{
    collect_expressions_from_schema, format_parsing_errors, format_validation_errors,
    log_schema_execution_start, parse_expressions_batch, validate_schema_basic,
};
...
fn test_expression_parsing_consolidation() {
    ...
    assert_eq!(
        single_expressions.len(),
        1,
        "Schema with one field should have 1 expression"
    );
...
fn test_error_handling_standardization() {
    ...
    assert!(
        ...
        "Formatted error should contain field name"
    );
    ...
    let parsing_errors = vec![
        (
            ...
            SchemaError::InvalidTransform("test".to_string()),
        ),
        (
            "content".to_string(),
            "Unknown function 'invalid_func'".to_string(),
            SchemaError::InvalidTransform("test".to_string()),
        ),
    ];
    let formatted_parsing = format_parsing_errors(&parsing_errors, "test_context");
    assert!(
        formatted_parsing.contains("title"),
        "Formatted parsing error should contain field name"
    );
}
...
fn test_comprehensive_deduplication_verification() {
    ...
    assert!(
        ...
        "Shared utilities should be consolidated"
    );
    ...
    assert_eq!(
        expressions.len(),
        1,
        "Expression collection should be consolidated"
    );
...
fn test_performance_characteristics() {
    ...
    assert!(
        ...
        "Consolidated functions should be performant"
    );
}
...
fn test_edge_cases_and_error_handling() {
    ...
    assert!(
        parsed_result.is_ok(),
        "Empty expressions should be handled gracefully"
    );
}
...
fn test_deduplication_acceptance_criteria() {
    ...
    assert!(
        validation_result.is_ok(),
        "Consolidated validation should work"
    );
    ...
    assert_eq!(
        expressions.len(),
        1,
        "Consolidated expression collection should work"
    );
    ...
    assert!(
        !formatted.is_empty(),
        "Standardized error handling should work"
    );
...
fn test_code_deduplication_metrics() {
    ...
    assert_eq!(
        expressions.len(),
        1,
        "Consolidated expression collection works"
    );
