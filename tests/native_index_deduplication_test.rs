...
use datafold::db_operations::{NativeIndexManager, ClassificationType};
...
type BatchIndexOperation = (String, String, KeyValue, serde_json::Value, Option<Vec<String>>);
...
fn test_deduplication_preserves_different_classifications() {
    ...
    manager.batch_index_field_values_with_classifications(&operations)
    ...
    let name_results = manager.search_with_classification("john smith", 
    ...
    let word_entries: Vec<_> = word_results.iter()
        ...
        .collect();
    assert_eq!(word_entries.len(), 1, "Should have exactly 1 word entry for record1");
    
    let name_entries: Vec<_> = name_results.iter()
        .filter(|r| r.key_value.hash.as_deref() == Some("record1"))
        .collect();
    assert_eq!(name_entries.len(), 1, "Should have exactly 1 name entry for record1");
...
fn test_deduplication_removes_duplicate_same_classification() {
    ...
    let learning_results = manager.search_word("learning").expect("search should succeed");
    eprintln!("Search results for 'learning': {} entries", learning_results.len());
    
    let learning_entries: Vec<_> = learning_results.iter()
        .filter(|r| r.key_value.hash.as_deref() == Some("record1"))
        .collect();
    
    assert_eq!(learning_entries.len(), 1, "Duplicate entries should be deduplicated");
...
fn test_deduplication_with_multiple_classifications_same_batch() {
    ...
    let email_results = manager.search_with_classification("alice@example.com",
    ...
    let username_results = manager.search_with_classification("alice@example.com",
        Some(ClassificationType::Username))
        .expect("username search should succeed");
    eprintln!("Username search results: {} entries", username_results.len());
    
    // Check email
    let email_entries: Vec<_> = email_results.iter()
        .filter(|r| r.key_value.hash.as_deref() == Some("record1"))
        .collect();
...
fn test_deduplication_across_different_fields_same_record() {
    ...
    let title_entries: Vec<_> = rust_results.iter()
        .filter(|r| r.key_value.hash.as_deref() == Some("record1") && r.field == "title")
        .collect();
    
    let content_entries: Vec<_> = rust_results.iter()
    ...
    assert_eq!(title_entries.len() + content_entries.len(), rust_results.len(),
...
fn test_deduplication_handles_same_field_different_records() {
    ...
    let shared_results = manager.search_word("shared").expect("search should succeed");
    eprintln!("Search results for 'shared': {} entries", shared_results.len());
    
    assert_eq!(shared_results.len(), 3, "Should have 3 entries (one per record)");
    
    let record_count_1 = shared_results.iter()
        .filter(|r| r.key_value.hash.as_deref() == Some("record1"))
        .count();
    let record_count_2 = shared_results.iter()
        .filter(|r| r.key_value.hash.as_deref() == Some("record2"))
        .count();
    let record_count_3 = shared_results.iter()
