...
fn create_transform_base_schema() -> Schema {
    ...
    schema
        .fields
        .insert("value1".to_string(), FieldVariant::Single(value1_field));
    ...
    schema
        .fields
        .insert("value2".to_string(), FieldVariant::Single(value2_field));

    println!(
        "✅ TransformBase schema created with fields: {:?}",
...
fn register_schema(fixture: &TestFixture, schema: &Schema) {
    fixture
        .db_ops
        .store_schema(&schema.name, schema)
        .expect("Failed to store schema for integration test");
}
...
fn mutate_field_value(
    ...
    println!(
        "🔄 Mutating {}.{} = {} (source: {})",
        schema_name, field_name, value, source
    );
    ...
    match fixture
        .db_ops
        .get_item::<datafold::atom::Molecule>(&molecule_key)
    {
        Ok(Some(molecule)) => {
            let atom_uuid = molecule.get_atom_uuid();
            println!(
                "🔍 DIAGNOSTIC: Molecule {} points to atom {}",
        ...
        Ok(None) => {
            println!(
                "❌ DIAGNOSTIC: Molecule {} not found in database",
                molecule_uuid
            );
        }
        Err(e) => {
            println!(
                "❌ DIAGNOSTIC: Error loading Molecule {}: {}",
                molecule_uuid, e
            );
        }
    }
...
fn query_field_value(
    ...
    println!(
        "🔍 Querying {}.{} (expecting: {})",
        schema.name, field_name, expected_value
    );

    // Use the query system to resolve field value
    let result =
        TransformUtils::resolve_field_value(&fixture.db_ops, schema, field_name, None, None)?;
...
fn test_single_field_mutation_to_query_flow() {
    ...
    register_schema(&fixture, &schema);

    // Step 1: Perform mutation on value1 field
    let test_value = json!(42);
    let molecule_uuid = mutate_field_value(
        &fixture,
        "TransformBase",
        "value1",
        test_value.clone(),
...
fn test_multiple_field_mutations_and_queries() {
    ...
    for (field_name, expected_value, _) in &test_cases {
        ...
        assert_eq!(
            ...
            "Field {} should return correct value",
            field_name
        );
    }
...
fn test_multiple_mutation_cycles_on_same_field() {
    ...
    for (i, (value, source)) in mutation_cycles.iter().enumerate() {
        ...
        // Perform mutation
        let molecule_uuid =
            mutate_field_value(&fixture, "TransformBase", "value1", value.clone(), source)
        ...
        assert_eq!(
            &result,
            value,
            "Cycle {}: Query should return latest mutated value",
            i + 1
        );
        ...
        if i > 0 {
            ...
            assert_eq!(
                molecule_uuids[i],
                molecule_uuids[0],
                "Cycle {}: Should reuse same Molecule UUID",
                i + 1
            );
        }
...
fn test_mutation_query_flow_with_different_data_types() {
    ...
    for (data_type, value) in test_data_types {
        ...
        assert_eq!(
            result, value,
            "Data type {}: Query should return correct value",
            data_type
        );

        println!(
            "✅ Data type {} test passed - Molecule: {}",
            data_type, molecule_uuid
        );
    }
...
fn test_concurrent_mutations_and_queries() {
    ...
    let mutation_handles: Vec<_> = (0..5)
        .map(|i| {
    ...
    for (i, success, molecule_uuid, _value) in &results {
        ...
        assert!(
            molecule_uuid.is_some(),
            "Concurrent operation {} should return Molecule UUID",
            i
        );
        println!(
            "✅ Concurrent operation {} succeeded - Molecule: {:?}",
            i, molecule_uuid
        );
    }
    ...
    let final_result =
        TransformUtils::resolve_field_value(&fixture.db_ops, &schema, "value1", None, None)
            .expect("Failed to query final state");

    println!(
        "✅ Final field state after concurrent operations: {}",
        final_result
    );
...
fn test_complete_range_only_architecture_validation() {
    ...
    for (field_name, field_variant) in &schema.fields {
        match field_variant {
            FieldVariant::Single(_) => {
                println!(
                    "✅ Field '{}' is Single (Range-only architecture compatible)",
                    field_name
                );
            }
            FieldVariant::Range(_) => {
                println!(
                    "✅ Field '{}' is Range (Range-only architecture compatible)",
                    field_name
                );
            }
            FieldVariant::HashRange(_) => {
                println!(
                    "✅ Field '{}' is HashRange (Range-only architecture compatible)",
                    field_name
                );
    ...
    assert_eq!(
        result, test_value,
        "Range-only architecture should handle mutations and queries correctly"
    );
...
fn test_diagnostic_molecule_bug_prevention() {
    ...
    let dynamic_molecule_uuid = mutate_field_value(
        &fixture,
        "TransformBase",
        "value1",
        test_value.clone(),
    ...
    println!(
        "   Query correctly used dynamic Molecule: {}",
...
fn test_complete_mutation_query_integration_workflow() {
    ...
    mutate_field_value(
        ...
        "TransformBase",
        "value1",
        complex_update.clone(),
        "complex_phase",
    )
    .expect("Failed complex mutation");

    query_field_value(&fixture, &schema, "value1", &complex_update).expect("Failed complex query");
    ...
    let final_value2 =
        TransformUtils::resolve_field_value(&fixture.db_ops, &schema, "value2", None, None)
            .expect("Failed final validation query for value2");

    assert_eq!(
        final_value1, complex_update,
        "Final value1 should match complex update"
    );
    assert_eq!(
        final_value2,
        json!("rapid_cycle_3"),
        "Final value2 should match last rapid cycle"
    );
