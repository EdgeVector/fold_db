...
impl HashRangeEndToEndTestFixture {
    ...
    /// Wait for a mutation to complete with a timeout and context-aware logging
    async fn wait_for_mutation_completion(
        &self,
        mutation_id: &str,
        context: &str,
    ) -> Result<(), Box<dyn std::error::Error>> {
        println!(
            "⏳ Waiting for mutation {} to complete ({}).",
            mutation_id, context
        );
        ...
        .await
        {
            ...
            Ok(Err(e)) => {
                println!(
                    "❌ Mutation {} failed while waiting ({}): {}",
                    mutation_id, context, e
                );
                Err(e.into())
            }
            Err(_elapsed) => {
                let msg = format!("Mutation {} timed out after 30s ({})", mutation_id, context);
                println!("⏰ {}", msg);
                Err(msg.into())
            }
        }
    ...
    async fn load_and_approve_blogpost_schema(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.fold_db
            .load_schema_from_file("available_schemas/BlogPost.json")?;
    ...
    async fn create_test_blog_posts(&mut self) -> Result<Vec<String>, Box<dyn std::error::Error>> {
        ...
        for (i, post_data) in blog_posts.iter().enumerate() {
            ...
            let context = format!("blog post {}", i + 1);
            self.wait_for_mutation_completion(&mutation_id, &context)
                .await?;
        }

        println!("📊 Created {} blog posts", blog_posts.len());
        println!(
            "📊 Created {} blog posts with mutation IDs: {:?}",
            blog_posts.len(),
            mutation_ids
        );
        Ok(mutation_ids)
    ...
    async fn load_and_approve_word_index_schema(
        &mut self,
        ...
        self.fold_db.add_schema_available(schema).map_err(|e| {
        ...
        self.fold_db
            .approve_schema("BlogPostWordIndex")
            .map_err(|e| {
    ...
    async fn verify_transform_registration(
        &self,
        transform_id: &str,
    ) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (id, transform) in &transforms {
            println!(
                ...
                id, transform.output, transform.inputs
            );
        }

        // Extract the base transform name (without timestamp suffix)
        let base_transform_name = transform_id
            .split('.')
            .take(2)
            .collect::<Vec<_>>()
            .join(".");
        println!(
            "🔍 Looking for base transform name: {}",
            base_transform_name
        );
        ...
        if let Some(transform) = transforms.get(&base_transform_name) {
            println!(
                "✅ Found transform: {} -> {}",
                base_transform_name, transform.output
            );
    ...
    async fn trigger_transform_execution(
        &mut self,
        transform_id: &str,
    ) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn execute_stored_mutations(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for result in db_ops.db().iter().flatten() {
            ...
            if key_str.starts_with("mutation:") {
                ...
                match serde_json::from_str::<datafold::schema::types::operations::Mutation>(
                    &mutation_json,
                ) {
                    Ok(mutation) => {
                        println!("📝 Executing mutation for schema: {}", mutation.schema_name);
                        println!(
                            "🔍 DEBUG: Mutation content: {}",
                        ...
                        match self.fold_db.write_schema(mutation) {
                            Ok(mutation_id) => {
                                println!(
                                    "✅ Mutation executed successfully with ID: {}",
                                    mutation_id
                                );
                                mutation_count += 1;

                                // Ensure the mutation is fully processed before moving on
                                self.wait_for_mutation_completion(
                                    &mutation_id,
                                    "transform-generated mutation",
                                )
                                .await?;
                            }
                            Err(e) => {
    ...
    async fn query_and_validate_word_index(
        &self,
        test_words: Vec<&str>,
    ) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for word in test_words {
                        ...
                        match self.validate_query_result_format(&query_result, word) {
                            Ok(_) => {
                                ...
                                match self.validate_query_result_data(&query_result, word) {
                                    ...
                                    Err(e) => {
                                        println!(
                                            "❌ Query data validation failed for word '{}': {}",
                                            word, e
                                        );
                                        if retry_count < max_retries - 1 {
                                            println!(
                                                "🔄 Retrying query for word '{}' (attempt {}/{})",
                                                word,
                                                retry_count + 2,
                                                max_retries
                                            );
                            ...
                            Err(e) => {
                                println!(
                                    "❌ Query format validation failed for word '{}': {}",
                                    word, e
                                );
                                if retry_count < max_retries - 1 {
                                    println!(
            ...
            if !query_successful {
                return Err(format!(
                    "Query validation failed for word '{}' after {} retries",
                    word, max_retries
                )
                .into());
            }
    ...
    fn validate_query_result_format(
        &self,
        result: &Value,
        word: &str,
    ) -> Result<(), Box<dyn std::error::Error>> {
            ...
            if let Some(word_data) = result_obj.get(word) {
                if let Some(word_obj) = word_data.as_object() {
                    ...
                    if !has_valid_range_entries {
                        return Err(
                            format!("No valid range entries found for word '{}'", word).into()
                        );
                    }
                } else {
                                ...
                                if let Some(field_value) = range_obj.get(*field) {
                                    if field_value.is_null() {
                                        return Err(format!(
                                            ...
                                            field, word, range_key
                                        )
                                        .into());
                                    }
                                } else {
                                    return Err(format!(
                                        "Field '{}' missing for word '{}' in range '{}'",
                                        field, word, range_key
                                    )
                                    .into());
                                }
...
async fn test_hashrange_query_format_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fixture
        .load_and_approve_blogpost_schema()
        .await
        .map_err(|e| {
    ...
    let transform_id = fixture
        .load_and_approve_word_index_schema()
        .await
        .map_err(|e| {
    ...
    let mutation_ids = fixture.create_test_blog_posts().await.map_err(|e| {
    ...
    for (index, mutation_id) in mutation_ids.iter().enumerate() {
        let context = format!("BlogPost creation {}/{}", index + 1, mutation_ids.len());
        fixture
            .wait_for_mutation_completion(mutation_id, &context)
            .await
            .map_err(|e| {
    ...
    if let Some(result_obj) = query_result.as_object() {
        if let Some(content_field) = result_obj.get("content") {
            if let Some(content_array) = content_field.as_array() {
                ...
                println!(
                    "✅ Found {} range entries for 'DataFold' hash key",
                    content_array.len()
                );
            }
        }
