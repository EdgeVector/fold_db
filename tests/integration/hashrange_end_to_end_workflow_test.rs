...
use datafold::schema::types::{MutationType, Schema};
...
impl HashRangeEndToEndTestFixture {
    ...
    async fn load_and_approve_word_index_schema(
        ...
        let schema_json = std::fs::read_to_string(schema_path).map_err(|e| {
            println!(
                "❌ Failed to read BlogPostWordIndex schema file '{}': {}",
                schema_path, e
            );
            Box::<dyn std::error::Error>::from(e)
        })?;

        let declarative_schema: DeclarativeSchemaDefinition = serde_json::from_str(&schema_json)
            .map_err(|e| {
                println!(
                    "❌ Failed to parse BlogPostWordIndex declarative schema from '{}': {}",
                    schema_path, e
                );
                Box::<dyn std::error::Error>::from(e)
            })?;
        ...
        let schema: Schema = self
            ...
            .map_err(|e| {
                println!(
                    "❌ Failed to interpret BlogPostWordIndex declarative schema: {}",
                    e
                );
                Box::<dyn std::error::Error>::from(e)
            })?;
        ...
        println!(
            "🔍 Debug: Interpreted BlogPostWordIndex schema with fields: {:?}",
        ...
        self.fold_db
            ...
            .map_err(|e| {
                println!(
                    "❌ Failed to persist BlogPostWordIndex schema '{}': {}",
                    schema.name, e
                );
                Box::<dyn std::error::Error>::from(e)
            })?;
        ...
        self.fold_db
            .add_schema_available(schema.clone())
            .map_err(|e| {
                println!(
                    "❌ Failed to register BlogPostWordIndex schema '{}': {}",
                    schema.name, e
                );
                Box::<dyn std::error::Error>::from(e)
            })?;

        self.fold_db.approve_schema(&schema.name).map_err(|e| {
            println!(
                "❌ Failed to approve BlogPostWordIndex schema '{}': {}",
                schema.name, e
            );
            Box::<dyn std::error::Error>::from(e)
        })?;
        ...
        println!("🔄 Using transform ID: {}", transform_id);
        ...
        self.fold_db.reload_transforms().map_err(|e| {
