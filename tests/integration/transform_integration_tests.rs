...
fn test_error_recovery_and_edge_cases() {
    ...
    match result {
        Ok(json_result) => {
            println!(
                "Transform handled malformed data gracefully: {}",
                json_result
            );
...
fn test_performance_under_various_conditions() {
    ...
    for (i, transform_id) in transform_ids.iter().enumerate() {
        ...
        let result = TransformExecutor::execute_transform(&transforms[transform_id], input_values)
    ...
    assert!(
        registration_duration.as_millis() < 2000,
        "Registration took too long: {}ms",
        registration_duration.as_millis()
    );
    assert!(
        execution_duration.as_millis() < 2000,
    ...
    println!(
        "  Registration (5 transforms): {}ms",
        registration_duration.as_millis()
    );
    println!(
        "  Execution (5 transforms): {}ms",
        execution_duration.as_millis()
    );
}
...
fn test_validation_infrastructure_integration() {
    ...
    if alignment_result.valid {
        println!(
            "Field alignment validation passed with {} warnings",
        ...
        println!(
            "Field alignment validation failed with {} errors",
