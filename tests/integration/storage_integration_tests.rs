...
fn test_storage_integration_both_transform_types() {
    ...
    db_ops.store_transform("procedural_test", &procedural_transform)
        .expect("Failed to store procedural transform");
    db_ops.store_transform("declarative_test", &declarative_transform)
        .expect("Failed to store declarative transform");
    
    // Retrieve both transforms
    let retrieved_procedural = db_ops.get_transform("procedural_test")
        .expect("Failed to get procedural transform")
        .expect("Procedural transform not found");
    
    let retrieved_declarative = db_ops.get_transform("declarative_test")
    ...
    assert!(matches!(retrieved_declarative.kind, TransformKind::Declarative { .. }));
...
fn test_transform_manager_integration() {
    ...
    transform_manager.register_transform_event_driven(procedural_registration)
    ...
    transform_manager.register_transform_event_driven(declarative_registration)
        .expect("Failed to register declarative transform");
    
    // Verify both transforms are registered
    let transforms = transform_manager.list_transforms().expect("Failed to list transforms");
    ...
    let procedural_transforms = transform_manager.get_transforms_for_field("test_schema", "input_field")
        .expect("Failed to get transforms for field");
    assert!(procedural_transforms.contains(&"procedural_double".to_string()));
    
    let declarative_transforms = transform_manager.get_transforms_for_field("test_schema", "data")
        .expect("Failed to get transforms for field");
...
fn test_backward_compatibility_procedural_transforms() {
    ...
    db_ops.store_transform("legacy_procedural", &legacy_transform)
        .expect("Failed to store legacy transform");
    
    // Retrieve and verify it works
    let retrieved = db_ops.get_transform("legacy_procedural")
...
fn test_storage_performance_both_types() {
    ...
    for i in 0..10 {
        ...
        db_ops.store_transform(&format!("procedural_{}", i), &procedural_transform)
            .expect("Failed to store procedural transform");
        ...
        let declarative_schema = DeclarativeSchemaDefinition {
            ...
            schema_type: SchemaType::Single,
            fields: HashMap::from([
                (format!("result{}", i), FieldDefinition {
                    field_type: Some("single".to_string()),
        ...
        db_ops.store_transform(&format!("declarative_{}", i), &declarative_transform)
            .expect("Failed to store declarative transform");
    }
    ...
    for i in 0..10 {
        ...
        let declarative = db_ops.get_transform(&format!("declarative_{}", i))
            .expect("Failed to get declarative transform")
            .expect("Declarative transform not found");
        assert!(matches!(declarative.kind, TransformKind::Declarative { .. }));
    }
    ...
    println!("Storage performance: {}ms store, {}ms retrieve", 
             store_duration.as_millis(), retrieve_duration.as_millis());
}
