...
impl RangeArchitectureTestFixture {
    ...
    fn create_test_range_schema(&self) -> Result<Schema, Box<dyn std::error::Error>> {
        let mut user_catalog_schema =
            Schema::new_range("UserCatalog".to_string(), "user_id".to_string());
        ...
        self.db_ops
            .store_schema("UserCatalog", &user_catalog_schema)?;
...
fn test_range_fields_as_collection_replacement() {
    ...
    let fixture = RangeArchitectureTestFixture::new().expect("Failed to create test fixture");

    let _schema = fixture
        .create_test_range_schema()
    ...
    fixture
        .populate_range_field_data(&mut user_catalog_field, user_data.clone())
    ...
    println!(
        "✅ Range field count operation works: {} items",
        user_catalog_field.count()
    );
    ...
    println!(
        "✅ Range field multiple lookup works: {} items",
        multi_result.total_count
    );
    ...
    println!(
        "✅ Range field range queries work: {} items in range",
    ...
    println!(
        "✅ Range field prefix search works: {} items with prefix",
        prefix_result.total_count
    );
...
fn test_range_field_lookup_hash_functionality() {
    ...
    fixture
        .populate_range_field_data(&mut product_catalog_field, product_data.clone())
        .expect("Failed to populate product catalog");

    println!(
        "✅ Created product catalog with {} items",
        product_catalog_field.count()
    );
    ...
    for i in 0..lookup_iterations {
        ...
        assert_eq!(
            ...
            "Should find exactly one product for {}",
            product_id
        );
    ...
    println!(
        "✅ Hash lookup performance: {} lookups in {:?}",
        lookup_iterations, lookup_duration
    );
    println!("   Average lookup time: {:?}", avg_lookup_time);

    // Performance assertion (should be sub-millisecond for hash lookups)
    assert!(
        avg_lookup_time < Duration::from_millis(1),
        "Hash lookups should be sub-millisecond"
    );
    ...
    let collision_test_data = vec![
        (
            "collision_test_001".to_string(),
            json!({"type": "collision_test", "id": 1}),
        ),
        (
            "collision_test_002".to_string(),
            json!({"type": "collision_test", "id": 2}),
        ),
        (
            "collision_test_003".to_string(),
            json!({"type": "collision_test", "id": 3}),
        ),
    ];

    let mut collision_field = FieldFactory::create_range_field();
    fixture
        .populate_range_field_data(&mut collision_field, collision_test_data.clone())
    ...
    for (key, _) in &collision_test_data {
        ...
        assert_eq!(
            result.total_count, 1,
            "Should find collision test item {}",
            key
        );
    ...
    fixture
        .populate_range_field_data(&mut large_field, large_dataset)
    ...
    println!(
        "✅ Memory efficiency: {} items created in {:?}",
        large_dataset_size, memory_duration
    );
    ...
    println!(
        "✅ Large dataset lookup: found item in {:?}",
        large_lookup_duration
    );
...
fn test_range_field_mutations_and_queries() {
    ...
    let query_data: Vec<(String, serde_json::Value)> = new_users
        .iter()
        .map(|(key, data)| (key.to_string(), data.clone()))
        .collect();

    fixture
        .populate_range_field_data(&mut query_field, query_data)
    ...
    println!(
        "✅ Range query: {} users in range",
        range_query_result.total_count
    );
    ...
    println!(
        "✅ Prefix query: {} users with prefix",
        prefix_query_result.total_count
    );
    ...
    println!(
        "✅ Pattern query: {} users matching pattern",
        pattern_query_result.total_count
    );
    ...
    println!(
        "✅ Pagination simulation: Page 1 has {} keys, Page 2 has {} keys",
        page_1_keys.len(),
        page_2_keys.len()
    );
...
fn test_range_field_performance_characteristics() {
    ...
    fixture
        .populate_range_field_data(&mut performance_field, performance_data)
    ...
    println!(
        "✅ Large dataset insertion: {} items in {:?}",
        large_dataset_size, insertion_duration
    );
    ...
    println!(
        "✅ Random access: {} lookups in {:?}",
        random_access_count, random_access_duration
    );
    println!(
        "   Random access rate: {:.2} lookups/second",
        random_access_rate
    );
    ...
    println!(
        "✅ Range scans: {} scans in {:?}",
        range_scan_count, range_scan_duration
    );
    ...
    assert_eq!(bulk_result.total_count, bulk_keys.len());
    println!(
        "✅ Bulk operations: {} items in {:?}",
        bulk_size, bulk_operation_duration
    );
    println!(
        "   Bulk operation rate: {:.2} items/second",
        bulk_operation_rate
    );
    ...
    fixture
        .populate_range_field_data(&mut memory_field, memory_data)
    ...
    println!(
        "✅ Memory test: {} items in {:?}",
        memory_test_size, memory_test_duration
    );
    ...
    println!(
        "✅ Large dataset verification: found item in {:?}",
    ...
    assert!(
        ...
        "Insertion rate should be > 100 items/sec"
    );
    assert!(
        random_access_rate > 100.0,
        "Random access rate should be > 100 lookups/sec"
    );
    assert!(
        bulk_operation_rate > 100.0,
        "Bulk operation rate should be > 100 items/sec"
    );
...
fn test_range_filtering_and_search_capabilities() {
    ...
    let search_test_data = vec![
        ...
        (
            "user_admin_001",
            json!({"role": "admin", "department": "engineering", "level": "senior"}),
        ),
        (
            "user_admin_002",
            json!({"role": "admin", "department": "marketing", "level": "junior"}),
        ),
        (
            "user_dev_001",
            json!({"role": "developer", "department": "engineering", "level": "senior"}),
        ),
        (
            "user_dev_002",
            json!({"role": "developer", "department": "engineering", "level": "mid"}),
        ),
        (
            "user_dev_003",
            json!({"role": "developer", "department": "engineering", "level": "junior"}),
        ),
        (
            "user_qa_001",
            json!({"role": "qa", "department": "engineering", "level": "senior"}),
        ),
        (
            "user_qa_002",
            json!({"role": "qa", "department": "engineering", "level": "mid"}),
        ),
        (
            "user_sales_001",
            json!({"role": "sales", "department": "sales", "level": "senior"}),
        ),
        (
            "user_sales_002",
            json!({"role": "sales", "department": "sales", "level": "junior"}),
        ),
        (
            "user_support_001",
            json!({"role": "support", "department": "customer_service", "level": "mid"}),
        ),
        // Product data with various patterns
        (
            "product_laptop_001",
            json!({"category": "electronics", "type": "laptop", "price": 999}),
        ),
        (
            "product_laptop_002",
            json!({"category": "electronics", "type": "laptop", "price": 1299}),
        ),
        (
            "product_phone_001",
            json!({"category": "electronics", "type": "phone", "price": 699}),
        ),
        (
            "product_phone_002",
            json!({"category": "electronics", "type": "phone", "price": 899}),
        ),
        (
            "product_book_001",
            json!({"category": "books", "type": "fiction", "price": 15}),
        ),
        (
            "product_book_002",
            json!({"category": "books", "type": "non-fiction", "price": 25}),
        ),
        (
            "product_clothing_001",
            json!({"category": "clothing", "type": "shirt", "price": 35}),
        ),
        (
            "product_clothing_002",
            json!({"category": "clothing", "type": "pants", "price": 65}),
        ),
    ];
    ...
    fixture
        .populate_range_field_data(&mut search_field, search_test_data_converted)
        .expect("Failed to populate search test data");

    println!(
        "✅ Created search dataset with {} items",
        search_field.count()
    );
    ...
    assert_eq!(multi_result.total_count, 3);
    println!(
        "✅ Multiple key lookup works: {} items",
        multi_result.total_count
    );
    ...
    let user_count = search_test_data
        .iter()
        .filter(|(key, _)| key.starts_with("user_"))
        .count();
    assert_eq!(user_prefix_result.total_count, user_count);
    println!(
        "✅ User prefix filter: {} users found",
        user_prefix_result.total_count
    );
    ...
    let product_count = search_test_data
        .iter()
        .filter(|(key, _)| key.starts_with("product_"))
        .count();
    assert_eq!(product_prefix_result.total_count, product_count);
    println!(
        "✅ Product prefix filter: {} products found",
        product_prefix_result.total_count
    );
    ...
    let admin_count = search_test_data
        .iter()
        .filter(|(key, _)| key.starts_with("user_admin_"))
        .count();
    assert_eq!(admin_prefix_result.total_count, admin_count);
    println!(
        "✅ Admin prefix filter: {} admins found",
        admin_prefix_result.total_count
    );
    ...
    println!(
        "✅ User range filter: {} items in range",
        user_range_result.total_count
    );
    ...
    println!(
        "✅ Product range filter: {} items in range",
        product_range_result.total_count
    );
    ...
    let laptop_count = search_test_data
        .iter()
        .filter(|(key, _)| key.contains("laptop"))
        .count();
    assert_eq!(laptop_pattern_result.total_count, laptop_count);
    println!(
        "✅ Laptop pattern filter: {} laptops found",
        laptop_pattern_result.total_count
    );
    ...
    let dev_count = search_test_data
        .iter()
        .filter(|(key, _)| key.contains("_dev_"))
        .count();
    assert_eq!(dev_pattern_result.total_count, dev_count);
    println!(
        "✅ Developer pattern filter: {} developers found",
        dev_pattern_result.total_count
    );
    ...
    println!(
        "✅ Category filtering: {} laptops, {} phones",
    ...
    println!(
        "✅ Filter performance: {} filters in {:?}",
        filter_iterations, filter_performance_duration
    );
...
fn test_range_schema_validation_and_consistency() {
    ...
    let mut valid_range_schema =
        Schema::new_range("ValidRangeSchema".to_string(), "timestamp".to_string());
    ...
    fixture
        .db_ops
        .store_schema("ValidRangeSchema", &valid_range_schema)
        .expect("Failed to store valid Range schema");

    let retrieved_schema = fixture
        .db_ops
        .get_schema("ValidRangeSchema")
    ...
    for (field_name, field_variant) in &retrieved_schema.fields {
        match field_variant {
            ...
            _ => panic!(
                "Field '{}' should be a Range field in RangeSchema",
                field_name
            ),
        }
    }
    ...
    fixture
        .populate_range_field_data(&mut timestamp_field, timestamp_data.clone())
        .expect("Failed to populate timestamp field");
    fixture
        .populate_range_field_data(&mut event_field, event_data.clone())
    ...
    for key in &timestamp_keys {
        assert!(
            ...
            "Event data should exist for timestamp key {}",
            key
        );
    }
    ...
    assert_eq!(
        ...
        event_range_result.total_count
    );

    // Verify the same keys are returned
    for key in timestamp_range_result.matches.keys() {
        assert!(
            event_range_result.matches.contains_key(key),
            "Event field should contain key {} found in timestamp field",
            key
        );
    }

    println!(
        "✅ Range query consistency verified: {} items in both fields",
        timestamp_range_result.total_count
    );
    ...
    fixture
        .db_ops
        .store_schema("ValidRangeSchemaV2", &evolved_schema)
        .expect("Failed to store evolved schema");

    let evolved_retrieved = fixture
        .db_ops
        .get_schema("ValidRangeSchemaV2")
    ...
    println!(
        "✅ Consistency performance: {} operations in {:?}",
        consistency_iterations * 2,
        consistency_duration
    );
    println!(
        "   Consistency rate: {:.2} operations/second",
        consistency_rate
    );

    assert!(
        consistency_rate > 50.0,
        "Consistency operation rate should be > 50 ops/sec"
    );
