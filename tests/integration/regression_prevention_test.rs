...
use datafold::schema::{field_factory::FieldFactory, types::field::FieldVariant, Schema};
use serde_json::json;
use std::sync::{
...
impl RegressionPreventionTestFixture {
    ...
    fn create_schema_with_static_molecule(&self) -> Result<Schema, Box<dyn std::error::Error>> {
        ...
        self.db_ops
            .store_schema("ProblematicSchema", &problematic_schema)?;
    ...
    fn validate_molecule_lifecycle(
        ...
        let initial_molecule_uuid = initial_result
            .molecule_uuid
        ...
        let update_molecule_uuid = update_result
            .molecule_uuid
        ...
        if let Some(prev_atom_uuid) = updated_atom.prev_atom_uuid() {
            if prev_atom_uuid != initial_atom_uuid {
                return Err(
                    "Atom chain is broken: prev_atom_uuid doesn't match initial atom".into(),
                );
            }
        }
    ...
    fn validate_molecule_exists(
        &self,
        molecule_uuid: &str,
    ) -> Result<Molecule, Box<dyn std::error::Error>> {
        self.db_ops
            .get_item::<Molecule>(&format!("ref:{}", molecule_uuid))?
...
fn test_molecule_resolution_bug_prevention() {
    ...
    let fixture = RegressionPreventionTestFixture::new().expect("Failed to create test fixture");

    // Test 1: Static Molecule override prevention
    println!("🔧 Test 1: Static Molecule Override Prevention");

    let schema = fixture
        .create_schema_with_static_molecule()
    ...
    if let Some(FieldVariant::Single(field)) = schema.fields.get("problematic_field") {
        assert_eq!(
            field.molecule_uuid(),
            Some(&"STATIC_REF_SHOULD_BE_OVERRIDDEN".to_string())
        );
    ...
    let mutation_result = fixture
        .execute_tracked_mutation(
            "ProblematicSchema",
            "problematic_field",
            test_value.clone(),
            "bug_prevention_test",
    ...
    assert!(
        ...
        "Mutation should succeed despite static Molecule"
    );

    let dynamic_molecule_uuid = mutation_result
        .molecule_uuid
    ...
    assert_ne!(
        dynamic_molecule_uuid, "STATIC_REF_SHOULD_BE_OVERRIDDEN",
    ...
    let dynamic_molecule = fixture
        .validate_molecule_exists(&dynamic_molecule_uuid)
        .expect("Dynamic Molecule should exist");

    let dynamic_atom = fixture
        .validate_atom_exists(dynamic_molecule.get_atom_uuid())
    ...
    let second_molecule_uuid = second_mutation
        .molecule_uuid
        .expect("Second mutation should return Molecule UUID");

    // Should reuse the same dynamic Molecule
    assert_eq!(
        dynamic_molecule_uuid, second_molecule_uuid,
    ...
    let normal_molecule_uuid = normal_mutation
        .molecule_uuid
        .expect("Normal mutation should return Molecule UUID");

    // Normal field should get its own Molecule
    assert_ne!(
        normal_molecule_uuid, dynamic_molecule_uuid,
...
fn test_transform_trigger_bug_prevention() {
    ...
    fixture
        .db_ops
        .store_schema("TransformSource", &source_schema)
    ...
    fixture
        .db_ops
        .store_schema("TransformTarget", &target_schema)
        ...
        match executed_consumer.recv_timeout(Duration::from_millis(TEST_WAIT_MS)) {
            Ok(executed_event) => {
                executed_transforms
                    .lock()
                    .unwrap()
                    .push(executed_event.transform_id);
            }
    ...
    println!(
        "✅ Transform trigger handling: {} triggered, {} executed",
    ...
    while order_start.elapsed() < order_timeout
        && received_order.lock().unwrap().len() < order_test_count
    {
    ...
    println!(
        "✅ Ordering test: expected {} events, received {}",
        expected_order.len(),
        actual_order.len()
    );
    ...
    let trigger_handles: Vec<_> = (0..trigger_threads)
        .map(|thread_id| {
    ...
    let concurrent_trigger_rate =
        concurrent_triggered_count as f64 / concurrent_duration.as_secs_f64();

    println!(
        "✅ Concurrent trigger handling: {} triggers in {:?}",
        concurrent_triggered_count, concurrent_duration
    );
    println!(
        "   Trigger rate: {:.2} triggers/sec",
        concurrent_trigger_rate
    );
    ...
    assert!(
        ...
        "Message publishing should succeed even for invalid transforms"
    );
    ...
    assert!(
        ...
        "System should recover from invalid triggers"
    );
    ...
    assert!(
        concurrent_trigger_rate > 10.0,
        "Concurrent trigger rate should be > 10/sec"
    );
...
fn test_edge_cases_prevention() {
    ...
    fixture
        .db_ops
        .store_schema("EdgeCaseSchema", &edge_case_schema)
    ...
    for (description, value) in edge_values {
        ...
        assert!(
            result.success,
            "Edge case {} should be handled correctly",
            description
        );
        ...
        assert_eq!(
            query_result, value,
            "Edge case {} should round-trip correctly",
            description
        );
    ...
    assert!(
        ...
        "Deeply nested data should be handled correctly"
    );
    ...
    println!(
        "✅ Rapid sequential mutations: {} mutations in {:?}",
    ...
    assert!(
        rapid_rate > 8.0,
        "Rapid mutation rate should be > 8/sec (allowing for system variance)"
    );
...
fn test_event_ordering_and_timing_safeguards() {
    ...
    fixture
        .db_ops
        .store_schema("TimingTestSchema", &timing_schema)
    ...
    while response_start.elapsed() < response_timeout
        && received_events.lock().unwrap().len() < sequence_length
    {
    ...
    println!(
        "✅ Sequential ordering: sent {}, received {}",
    ...
    let concurrent_handles: Vec<_> = (0..concurrent_threads)
        .map(|thread_id| {
            let message_bus = Arc::clone(&fixture.message_bus);
    ...
    let concurrent_success_rate =
        concurrent_successful_count as f64 / concurrent_total_count as f64;

    println!(
        "✅ Concurrent handling: {} successful / {} total ({:.1}%)",
        concurrent_successful_count,
        concurrent_total_count,
    ...
    while response_collection_start.elapsed() < response_collection_timeout {
        match burst_consumer.recv_timeout(Duration::from_millis(50)) {
            ...
            Err(_) => {
                ...
                if burst_responses.load(Ordering::Relaxed)
                    >= burst_events.load(Ordering::Relaxed) / 2
                {
                    break;
                }
            }
    ...
    println!(
        "✅ Burst handling: {} events sent, {} responses received",
    ...
    println!(
        ...
        if medium_timeout_result.is_ok() {
            "received"
        } else {
            "timeout"
        }
    ...
    // Performance assertions
    assert!(
        concurrent_success_rate > 0.5,
        "Concurrent success rate should be > 50%"
    );
    assert!(
        burst_send_rate > 100.0,
        "Burst send rate should be > 100 events/sec"
    );
...
fn test_data_consistency_safeguards() {
    ...
    fixture
        .db_ops
        .store_schema("ConsistencyTestSchema", &consistency_schema)
    ...
    let lifecycle_result = fixture
        .validate_molecule_lifecycle(
    ...
    assert!(
        lifecycle_result.molecule_reused,
        "Molecule should be reused"
    );
    assert!(
        lifecycle_result.atom_chain_valid,
        "Atom chain should be valid"
    );

    println!("✅ Molecule lifecycle consistency validated");
    println!(
        "   Initial Molecule: {}",
        lifecycle_result.initial_molecule_uuid
    );
    println!(
        "   Update Molecule: {}",
        lifecycle_result.update_molecule_uuid
    );
    ...
    let stored_schema = fixture
        .db_ops
        .get_schema("ConsistencyTestSchema")
        .expect("Failed to retrieve stored schema")
        .expect("Schema should exist after storage");
    ...
    let primary_molecule_uuid = primary_result
        .molecule_uuid
        .expect("Primary should have Molecule");
    let secondary_molecule_uuid = secondary_result
        .molecule_uuid
        .expect("Secondary should have Molecule");

    assert_ne!(
        primary_molecule_uuid, secondary_molecule_uuid,
    ...
    let consistency_handles: Vec<_> = (0..concurrent_threads)
        .map(|thread_id| {
            let message_bus = Arc::clone(&fixture.message_bus);
    ...
    let successful_mutations = consistency_results_vec
        .iter()
    ...
    let valid_molecules: Vec<_> = consistency_results_vec
        .iter()
        .filter_map(|(_, success, molecule_uuid)| {
            if *success {
                molecule_uuid.as_ref()
            } else {
                None
            }
        })
    ...
    if valid_molecules.len() > 1 {
        ...
        let all_same = valid_molecules
            .iter()
            .all(|molecule| *molecule == first_molecule);
        assert!(
            all_same,
            "All successful mutations should reuse the same Molecule"
        );
    }
    ...
    println!("   Duration: {:?}", concurrent_duration);
    println!(
        "   Molecule reuse validated: {} unique Molecules",
        valid_molecules
            .iter()
    ...
    // Performance assertions
    let concurrent_success_rate =
        successful_mutations as f64 / consistency_results_vec.len() as f64;
    assert!(
        concurrent_success_rate > 0.6,
        "Concurrent success rate should be > 60%"
    );
