...
fn test_end_to_end_declarative_transform_workflow() {
    ...
    let field_transforms = fixture.transform_manager.get_transforms_for_field("blog_processing", "blogpost")
        .expect("Failed to get transforms for field");
    
    assert!(field_transforms.contains(&"blog_processor".to_string()), 
...
fn test_hashrange_declarative_transform() {
    ...
    assert!(transforms.contains_key(&"word_indexer".to_string()), 
    ...
    let retrieved_schema = registered_transform.get_declarative_schema()
        .expect("Should have declarative schema");
    assert_eq!(retrieved_schema.name, "word_index");
    assert!(matches!(retrieved_schema.schema_type, SchemaType::HashRange));
}
...
fn test_multiple_declarative_transforms() {
    ...
    let schema1 = DeclarativeSchemaDefinition {
        ...
        fields: HashMap::from([
            ("output1".to_string(), FieldDefinition {
                field_type: Some("single".to_string()),
    ...
    fixture.transform_manager.register_transform_event_driven(registration1)
    ...
    let schema2 = DeclarativeSchemaDefinition {
        ...
        schema_type: SchemaType::Single,
        fields: HashMap::from([
            ("output2".to_string(), FieldDefinition {
                field_type: Some("single".to_string()),
    ...
    fixture.transform_manager.register_transform_event_driven(registration2)
    ...
    assert!(transforms.contains_key(&"declarative_1".to_string()), 
            "First transform should be registered");
    assert!(transforms.contains_key(&"declarative_2".to_string()), 
...
fn test_complex_field_dependencies() {
    ...
    let declarative_schema = DeclarativeSchemaDefinition {
        ...
        schema_type: SchemaType::HashRange,
        fields: HashMap::from([
            ("main_data".to_string(), FieldDefinition {
                field_type: Some("single".to_string()),
                atom_uuid: Some("source.map().data".to_string()),
            }),
            ("nested_info".to_string(), FieldDefinition {
                field_type: Some("single".to_string()),
                atom_uuid: Some("source.map().nested.field".to_string()),
            }),
            ("computed_value".to_string(), FieldDefinition {
                field_type: Some("single".to_string()),
    ...
    assert!(transforms.contains_key(&"complex_processor".to_string()), 
...
fn test_declarative_transform_persistence() {
    ...
    let declarative_schema = DeclarativeSchemaDefinition {
        ...
        schema_type: SchemaType::Single,
        fields: HashMap::from([
            ("persistent_output".to_string(), FieldDefinition {
                field_type: Some("single".to_string()),
    ...
    fixture.transform_manager.reload_transforms()
        .expect("Failed to reload transforms");
    
    // Verify transform is still available after reload
    let transforms_after_reload = fixture.transform_manager.list_transforms()
    ...
    let reloaded_schema = reloaded_transform.get_declarative_schema()
...
fn test_declarative_transform_schema_types() {
    ...
    let single_schema = DeclarativeSchemaDefinition {
        ...
        schema_type: SchemaType::Single,
        fields: HashMap::from([
            ("single_output".to_string(), FieldDefinition {
                field_type: Some("single".to_string()),
    ...
    fixture.transform_manager.register_transform_event_driven(single_registration)
    ...
    let range_schema = DeclarativeSchemaDefinition {
        ...
        fields: HashMap::from([
            ...
            ("range_output".to_string(), FieldDefinition {
                field_type: Some("single".to_string()),
    ...
    fixture.transform_manager.register_transform_event_driven(range_registration)
    ...
    let single_retrieved = transforms["single_test"].get_declarative_schema()
        .expect("Should have declarative schema");
    assert!(matches!(single_retrieved.schema_type, SchemaType::Single));
    
    let range_retrieved = transforms["range_test"].get_declarative_schema()
        .expect("Should have declarative schema");
    assert!(matches!(range_retrieved.schema_type, SchemaType::Range { .. }));
}
