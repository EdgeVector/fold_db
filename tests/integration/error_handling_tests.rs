...
impl ErrorHandlingFixture {
    fn new() -> Self {
        ...
        let transform_manager = Arc::new(
            TransformManager::new(db_ops.clone(), message_bus.clone())
...
fn test_validation_failures_malformed_declarative_transforms() {
    ...
    if let Err(error) = validation_result {
        println!(
            "Correctly failed validation for empty schema name: {}",
            error
        );
    }
    ...
    assert!(
        empty_fields_validation.is_err(),
...
fn test_error_messages_clear_and_actionable() {
    ...
    for invalid_expr in &invalid_expressions {
        ...
        assert!(
            result.is_err(),
            "Should fail to parse invalid expression: {}",
            invalid_expr
        );
    ...
    let mismatched_chains = vec![
        fixture
            .chain_parser
            .parse("blogpost.map().content")
            .expect("Failed to parse valid chain"),
        fixture
            .chain_parser
            .parse("blogpost.map().author")
            .expect("Failed to parse valid chain"),
    ];

    let alignment_result = fixture
        .field_alignment_validator
        .validate_alignment(&mismatched_chains)
    ...
    if alignment_result.valid {
        for warning in &alignment_result.warnings {
            ...
            assert!(
                warning_msg.contains("depth")
                    || warning_msg.contains("alignment")
                    || warning_msg.contains("mismatch")
                    || warning_msg.contains("performance")
                    || warning_msg.contains("optimization")
                    || warning_msg.contains("reducer"),
                "Warning message should be descriptive: {}",
                warning_msg
            );
...
fn test_edge_cases_complex_iterator_expressions() {
    ...
    for deep_expr in &deep_expressions {
        ...
        match result {
            Ok(parsed_chain) => {
                ...
                let alignment_result = fixture
                    .field_alignment_validator
                    .validate_alignment(&chains)
                    .expect("Failed to validate field alignment");

                if alignment_result.valid {
                    println!(
                        "Deep expression validation passed with {} warnings",
                        alignment_result.warnings.len()
                    );
                } else {
                    println!(
                        "Deep expression validation failed with {} errors",
                        alignment_result.errors.len()
                    );
            ...
            Err(error) => {
                println!(
                    "Deep expression parsing failed (acceptable): {} - {}",
                    deep_expr, error
                );
    ...
    for expr in &edge_case_expressions {
        ...
        assert!(
            result.is_ok(),
            "Edge case expression should parse: {}",
            expr
        );
...
fn test_error_recovery_and_system_stability() {
    ...
    assert!(
        validation_result.is_err(),
        "Invalid transform should fail validation"
    );

    // System should remain stable after validation failure
    let _transforms = fixture
        .transform_manager
        .list_transforms()
...
fn test_resource_exhaustion_scenarios() {
    ...
    println!(
        "Concurrent operations completed: {} successes, {} failures",
...
fn test_concurrent_error_scenarios() {
    ...
    println!(
        "Concurrent error scenarios completed: {} successes, {} failures",
        success_count, failure_count
    );
    ...
    println!(
        "Concurrent operations: {} successes, {} failures",
        success_count, failure_count
    );
