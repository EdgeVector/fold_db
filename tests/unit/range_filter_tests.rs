use datafold::schema::types::field::range_filter::{RangeFilter, matches_pattern};
...
fn test_range_filter_enum_serialization() {
    ...
    let deserialized: RangeFilter = serde_json::from_str(&serialized).unwrap();
    assert!(matches!(deserialized, RangeFilter::KeyRange { start, end } if start == "a" && end == "z"));
...
fn test_range_field_filter_application() {
    ...
    println!("✅ KeyPrefix filter works: found {} matches", result.total_count);
    ...
    println!("✅ KeyRange filter works: found {} matches", result.total_count);
    ...
    assert!(result.matches.contains_key("test_key"));
    assert!(result.matches.contains_key("test_other"));
    println!("✅ KeyPattern filter works: found {} matches", result.total_count);
    ...
    assert_eq!(result.total_count, 1);
    assert!(result.matches.contains_key("key1"));
    println!("✅ Value filter works: found {} matches", result.total_count);
}
...
fn test_range_field_get_keys_in_range() {
    ...
    println!("✅ get_keys_in_range works: found {} keys in range", keys_in_range.len());
}
...
fn test_range_field_count() {
    ...
    println!("✅ count functionality works: {} items", range_field.count());
}
...
fn test_range_filter_performance_characteristics() {
    ...
    println!("✅ KeyPrefix filter performance: {:?} for 1000 entries", duration);
    ...
    println!("✅ KeyRange filter performance: {:?} for 1000 entries", duration);
    ...
    println!("✅ KeyPattern filter performance: {:?} for 1000 entries", duration);
}
...
fn test_range_filter_serialization_round_trip() {
    ...
    let test_filters = vec![
        ...
        RangeFilter::Keys(vec!["key1".to_string(), "key2".to_string(), "key3".to_string()]),
