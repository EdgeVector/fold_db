...
use datafold::transform::iterator_stack::types::{
    ActiveScope, IteratorConfig, IteratorStack, IteratorState, IteratorType, MemoryHint,
};
...
fn test_scope_management_basic() {
    ...
    let scope = create_test_scope(
        0,
        IteratorType::Schema {
            field_name: TEST_FIELD_NAME.to_string(),
        },
        None,
    );
...
fn test_value_storage_and_retrieval_single_scope() {
    ...
    assert!(stack
        .set_current_value("test_key".to_string(), test_value.clone())
        .is_ok());
...
fn test_value_storage_and_retrieval_multiple_scopes() {
    ...
    stack
        .set_current_value("shared_key".to_string(), parent_value.clone())
        .unwrap();

    // Pop and push child scope to set value at depth 1
    stack.pop_scope().unwrap();
    stack
        .push_scope(create_test_scope(
            1,
            IteratorType::ArraySplit {
                field_name: "child".to_string(),
            },
        ...
        .unwrap();
    stack
        .set_current_value("shared_key".to_string(), child_value.clone())
        .unwrap();
...
fn test_context_at_depth_access() {
    ...
    assert_eq!(
        context.values.get("test").unwrap(),
...
fn test_iterator_state_management() {
    ...
    let iterator_state = IteratorState {
        ...
        items: vec![serde_json::json!("item1"), serde_json::json!("item2")],
        completed: false,
        error: None,
    };
    ...
    let context = stack.context_at_depth(0).unwrap();
    assert_eq!(
        context.iterator_state.current_item,
        Some(serde_json::json!("item1"))
    );
...
fn test_pop_scope_cleanup() {
    ...
    stack
        .set_current_value("test_key".to_string(), serde_json::json!("test_value"))
        .unwrap();
...
fn test_iterator_stack_equality() {
    ...
    let different_scope = create_test_scope(
        1,
        IteratorType::ArraySplit {
            field_name: "different".to_string(),
        },
        Some(0),
    );
