...
fn test_simple_field_access_parsing() {
    ...
    assert_eq!(
        obj.get("blog_title"),
        Some(&JsonValue::String("My Blog Post".to_string()))
    );
}
...
fn test_chain_expression_with_map() {
    ...
    assert_eq!(
        ...
        Some(&JsonValue::String("Parsed chain content".to_string()))
    );
}
...
fn test_special_field_in_chain() {
    ...
    assert_eq!(
        obj.get("uuid_field"),
        Some(&JsonValue::String("special-uuid-12345".to_string()))
    );
}
    ...
    match result {
        ...
        Err(err) => {
            ...
            assert!(
                ...
                "Should handle parsing errors gracefully without crashes: {}",
                error_msg
            );
        }
    }
...
fn test_chain_with_reducer_function() {
    ...
    match result {
        ...
        Err(err) => {
            ...
            let error_msg = format!("{:?}", err);
            assert!(
                error_msg.contains("Field alignment validation failed")
...
fn test_empty_chain_expression() {
    ...
    input_values.insert(
        "data".to_string(),
        JsonValue::String("Simple data value".to_string()),
    );
    ...
    let obj = json_result.as_object().unwrap();
    assert_eq!(
        obj.get("simple_field"),
        Some(&JsonValue::String("Simple data value".to_string()))
    );
}
