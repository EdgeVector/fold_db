...
fn test_hashrange_compound_key_structure() {
    ...
    input_values.insert(
        ...
        JsonValue::Array(vec![
            JsonValue::Object(serde_json::Map::from_iter(vec![
                ...
                (
                    "timestamp".to_string(),
                    JsonValue::String("2025-01-01T10:00:00Z".to_string()),
                ),
                (
                    "title".to_string(),
                    JsonValue::String("First Post".to_string()),
                ),
                (
                    "content".to_string(),
                    JsonValue::String("Content 1".to_string()),
                ),
            ])),
            JsonValue::Object(serde_json::Map::from_iter(vec![
                ...
                (
                    "timestamp".to_string(),
                    JsonValue::String("2025-01-02T10:00:00Z".to_string()),
                ),
                (
                    "title".to_string(),
                    JsonValue::String("Second Post".to_string()),
                ),
                (
                    "content".to_string(),
                    JsonValue::String("Content 2".to_string()),
                ),
            ])),
        ]),
    );
    ...
    match result {
        Ok(json_result) => {
            ...
            assert!(
                !result_obj.contains_key("_hash_field"),
                "Result should NOT contain _hash_field"
            );
            assert!(
                !result_obj.contains_key("_range_field"),
                "Result should NOT contain _range_field"
            );
            ...
            assert!(
                ...
                "hash_key array should not be empty"
            );
            assert!(
                !range_key_array.is_empty(),
                "range_key array should not be empty"
            );
            ...
            assert_eq!(
                hash_key_array[0].as_str().unwrap(),
                "Alice",
                "First hash_key should be 'Alice'"
            );
            assert_eq!(
                range_key_array[0].as_str().unwrap(),
                "2025-01-01T10:00:00Z",
                "First range_key should be '2025-01-01T10:00:00Z'"
            );
            assert_eq!(
                title_array[0].as_str().unwrap(),
                "First Post",
                "First title should be 'First Post'"
            );
            assert_eq!(
                content_array[0].as_str().unwrap(),
                "Content 1",
                "First content should be 'Content 1'"
            );
            ...
            assert_eq!(
                hash_key_array[1].as_str().unwrap(),
                "Bob",
                "Second hash_key should be 'Bob'"
            );
            assert_eq!(
                range_key_array[1].as_str().unwrap(),
                "2025-01-02T10:00:00Z",
                "Second range_key should be '2025-01-02T10:00:00Z'"
            );
            assert_eq!(
                title_array[1].as_str().unwrap(),
                "Second Post",
                "Second title should be 'Second Post'"
            );
            assert_eq!(
                content_array[1].as_str().unwrap(),
                "Content 2",
                "Second content should be 'Content 2'"
            );
...
fn test_hashrange_vs_regular_schema_distinction() {
    ...
    input_values.insert(
        ...
        JsonValue::Object(serde_json::Map::from_iter(vec![
            (
                "content".to_string(),
                JsonValue::String("Test content".to_string()),
            ),
            (
                "hash_key".to_string(),
                JsonValue::String("hash123".to_string()),
            ),
            (
                "range_key".to_string(),
                JsonValue::String("range456".to_string()),
            ),
        ])),
    );

    // Execute both transforms
    let hashrange_result =
        TransformExecutor::execute_transform(&hashrange_transform, input_values.clone());
    ...
    match (hashrange_result, regular_result) {
        (Ok(hashrange_json), Ok(regular_json)) => {
            ...
            assert!(
                !hashrange_obj.contains_key("_hash_field"),
                "HashRange should NOT have _hash_field"
            );
            assert!(
                !hashrange_obj.contains_key("_range_field"),
                "HashRange should NOT have _range_field"
            );
