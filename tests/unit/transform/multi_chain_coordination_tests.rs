...
fn test_basic_hashrange_schema_execution() {
    ...
    assert!(!obj.contains_key("_hash_field"), "Key fields should not be in output");
    assert!(!obj.contains_key("_range_field"), "Key fields should not be in output");
}
    ...
    match result {
        ...
        Err(err) => {
            ...
            assert!(!error_msg.contains("panic") && !error_msg.contains("crash"),
                   "Should handle ExecutionEngine limitations gracefully: {}", error_msg);
        }
    }
}
...
fn test_hashrange_schema_missing_key_config() {
    ...
    assert!(error_msg.contains("key configuration") || error_msg.contains("hash_field") || error_msg.contains("range_field"),
           "Error should mention missing key configuration: {}", error_msg);
}
...
fn test_multi_chain_coordination_with_different_depths() {
    ...
    fields.insert("mapped_field".to_string(), FieldDefinition {
    ...
    match result {
        ...
        Err(err) => {
            ...
            let error_msg = format!("{:?}", err);
            assert!(error_msg.contains("alignment") || error_msg.contains("depth") || 
                   error_msg.contains("CartesianProduct") || error_msg.contains("No current scope") ||
                   error_msg.contains("Execution error"),
                   "Error should be coordination-related: {}", error_msg);
        }
    }
}
...
fn test_multi_chain_parsing_errors() {
    ...
    match result {
        ...
        Err(err) => {
            ...
            let error_msg = format!("{:?}", err);
            assert!(!error_msg.contains("panic") && !error_msg.contains("crash"),
                   "Should handle parsing errors gracefully: {}", error_msg);
        }
    }
}
...
fn test_multi_chain_empty_expressions() {
    ...
    match result {
        ...
        Err(err) => {
            ...
            let error_msg = format!("{:?}", err);
            assert!(!error_msg.contains("panic") && !error_msg.contains("crash"),
                   "Should handle minimal expressions gracefully: {}", error_msg);
        }
    }
}
...
fn test_multi_chain_complex_expressions() {
    ...
    match result {
        ...
        Err(err) => {
            ...
            let error_msg = format!("{:?}", err);
            assert!(!error_msg.contains("panic") && !error_msg.contains("crash"),
                   "Should handle complex expressions gracefully: {}", error_msg);
        }
    }
}
...
fn test_multi_chain_coordination_logging() {
    ...
    let mut fields = HashMap::new();
    fields.insert("logged_field".to_string(), FieldDefinition {
        atom_uuid: Some("data.value".to_string()),
        field_type: Some("String".to_string()),
    });
