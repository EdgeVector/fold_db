...
fn test_range_schema_missing_range_key_field() {
    ...
    let mut fields = HashMap::new();
    fields.insert("other_field".to_string(), FieldDefinition {
...
fn test_range_schema_field_without_atom_uuid() {
    ...
    let error = result.unwrap_err();
    let error_msg = format!("{:?}", error);
    assert!(error_msg.contains("atom_uuid") || error_msg.contains("expression"),
           "Error should mention missing atom_uuid expression: {}", error_msg);
}
...
fn test_range_schema_with_complex_expressions() {
    ...
    fields.insert("word_count".to_string(), FieldDefinition {
    ...
    match result {
        ...
        Err(err) => {
            ...
            let error_msg = format!("{:?}", err);
            assert!(!error_msg.contains("panic") && !error_msg.contains("crash"),
                   "Should handle complex Range expressions gracefully: {}", error_msg);
        }
    }
}
...
fn test_range_schema_with_single_field() {
    ...
    let mut fields = HashMap::new();
    fields.insert("event_id".to_string(), FieldDefinition {
...
fn test_range_schema_error_handling() {
    ...
    match result {
        ...
        Err(err) => {
            ...
            let error_msg = format!("{:?}", err);
            assert!(!error_msg.contains("panic") && !error_msg.contains("crash"),
                   "Should handle Range schema errors gracefully: {}", error_msg);
        }
    }
}
...
fn test_range_vs_hashrange_distinction() {
    ...
    range_fields.insert("created_at".to_string(), FieldDefinition {
