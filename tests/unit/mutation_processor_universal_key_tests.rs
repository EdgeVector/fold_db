...
use datafold::schema::{SchemaCore, Field};
...
impl MutationProcessorTestFixture {
    fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        // Create schema core
        let schema_core = Arc::new(SchemaCore::new(db_path, db_ops.clone(), Arc::clone(&message_bus))?);
    ...
    fn create_hashrange_schema_with_universal_key(&self, hash_field: &str, range_field: &str) -> Schema {
    ...
    fn create_range_schema_with_universal_key(&self, range_field: &str) -> Schema {
        ...
        Schema {
            ...
            schema_type: SchemaType::Range { range_key: range_field.to_string() },
            key: Some(KeyConfig {
    ...
    fn create_single_schema_with_universal_key(&self, hash_field: &str, range_field: &str) -> Schema {
    ...
    fn create_mutation(&self, schema_name: &str, fields_and_values: HashMap<String, Value>) -> Mutation {
        Mutation {
...
fn test_hashrange_schema_missing_key_configuration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut schema = fixture.create_hashrange_schema_with_universal_key("hash_field", "range_field");
    ...
    let result = fixture.mutation_processor.process_field_mutations_via_service(
...
fn test_hashrange_schema_empty_key_fields() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(result.is_err());
    let error_msg = result.unwrap_err().to_string();
    assert!(error_msg.contains("HashRange schema 'TestHashRangeSchema' requires non-empty hash_field"));
