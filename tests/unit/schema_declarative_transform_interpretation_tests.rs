...
impl SchemaDeclarativeTransformTestFixture {
    fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let message_bus = std::sync::Arc::new(
            datafold::fold_db_core::infrastructure::message_bus::MessageBus::new(),
        );
...
fn test_hashrange_schema_interpretation_and_transform() {
    let fixture =
        SchemaDeclarativeTransformTestFixture::new().expect("Failed to create test fixture");
    ...
    let schema_result = fixture
        .schema_core
        .interpret_declarative_schema(declarative_schema.clone());
    assert!(
        schema_result.is_ok(),
    ...
    for field_name in ["blog", "author", "title"] {
        ...
        if let datafold::schema::types::field::FieldVariant::HashRange(hashrange_field) =
...
fn test_single_schema_interpretation_and_transform() {
    ...
    assert!(
        schema_result.is_ok(),
        "Single schema interpretation failed: {:?}",
        schema_result
    );
    ...
    for field_name in ["processed_content", "word_count", "author_name"] {
        assert!(schema.fields.contains_key(field_name));
        if let datafold::schema::types::field::FieldVariant::Single(single_field) =
            &schema.fields[field_name]
        {
            // Verify fields are not writable (derived from declarative schema)
...
fn test_range_schema_interpretation_and_transform() {
    ...
    assert!(
        schema_result.is_ok(),
        "Range schema interpretation failed: {:?}",
        schema_result
    );
...
fn test_multiple_blogposts_hash_range_mapping() {
    ...
    for (field_name, expected_expr) in expected_expressions {
        let field_def = retrieved_schema
            .fields
            .get(field_name)
    ...
    let key_config = retrieved_schema
        .key
        .as_ref()
        .expect("Should have key configuration");
    assert_eq!(
    ...
    println!(
        "   - Hash: 'This is the second blog post with different content'.split_by_word().map()"
    );
...
fn test_schema_validation_and_errors() {
    ...
    let schema_result = fixture
        .schema_core
        .interpret_declarative_schema(invalid_hashrange_schema);
    assert!(
        schema_result.is_err(),
        "Should fail for HashRange schema without key configuration"
    );
    ...
    let invalid_field_schema = DeclarativeSchemaDefinition {
        ...
        schema_type: SchemaType::Single,
        fields: HashMap::from([(
            "invalid_field_name_with_special_chars!@#".to_string(),
            FieldDefinition {
                // Invalid field name
                field_type: Some("single".to_string()),
                atom_uuid: Some("source.map().field1".to_string()),
    ...
    let schema_result = fixture
        .schema_core
        .interpret_declarative_schema(invalid_field_schema);
    ...
    let schema_result = fixture
        .schema_core
        .interpret_declarative_schema(valid_schema);
...
fn test_transform_registration_and_retrieval() {
    ...
    assert!(
        stored_transform.as_ref().unwrap().is_some(),
...
fn test_schema_field_mapping_and_expressions() {
    ...
    assert!(
        schema_result.is_ok(),
        "Expression test schema interpretation failed: {:?}",
        schema_result
    );
    ...
    for (field_name, expected_expr) in [
        ...
        (
            "transformed_field",
            "source.map().content.split_by_word().map()",
        ),
...
fn test_schema_type_conversion_and_field_variants() {
    ...
    assert!(
        schema_result.is_ok(),
        "Single schema conversion failed: {:?}",
        schema_result
    );
        ...
        &schema.fields["field1"]
    {
        assert!(!single_field.writable());
        assert_eq!(
            single_field.molecule_uuid(),
            Some(&"source.map().field1".to_string())
        );
    } else {
    ...
    let schema_result = fixture
        .schema_core
        .interpret_declarative_schema(hashrange_schema);
    assert!(
        schema_result.is_ok(),
        "HashRange schema conversion failed: {:?}",
        schema_result
    );
    ...
    assert!(
        schema_result.is_ok(),
        "Range schema conversion failed: {:?}",
        schema_result
    );
