...
fn create_test_schema() -> Schema {
    ...
    schema.fields.insert("email".to_string(), FieldVariant::Single(email_field));
...
fn create_test_mutation() -> Mutation {
    ...
    fields_and_values.insert("email".to_string(), Value::String("john@example.com".to_string()));
...
async fn test_wait_for_mutation_timeout_scenario() {
    ...
    let _receiver = completion_handler.register_mutation(mutation_id.clone()).await;
...
async fn test_wait_for_mutation_successful_completion() {
    ...
    tokio::spawn(async move {
        ...
        completion_handler_clone.signal_completion(&mutation_id_clone).await;
    });
...
async fn test_wait_for_mutation_with_schema_mutation() {
    ...
    fold_db.add_schema_available(schema).expect("Failed to add schema");
    fold_db.approve_schema("test_user_profile").expect("Failed to approve schema");
    
    // Create and execute a mutation to get a real mutation ID
    let mutation = create_test_mutation();
    let mutation_id = fold_db.write_schema(mutation).expect("Failed to write schema");
...
async fn test_multiple_concurrent_wait_for_mutation() {
    ...
    for i in 0..num_mutations {
        ...
        let handle = tokio::spawn(async move {
            fold_db_clone.wait_for_mutation(&mutation_id).await
        });
        handles.push(handle);
    }
    ...
    tokio::spawn(async move {
        ...
        for mutation_id in mutation_ids_clone {
            completion_handler_clone.signal_completion(&mutation_id).await;
        }
    });
...
async fn test_wait_for_mutation_async_behavior() {
    ...
    let wait_handle = tokio::spawn(async move {
        fold_db_clone.wait_for_mutation(&mutation_id_clone).await
    });
