...
use crate::declarative_transform_test_utils::{
    DeclarativeTransformTestFixture, TestAssertions, TestSchemaBuilder,
};
...
fn test_declarative_schema_parsing() {
    ...
    let schema_result = fixture
        .schema_core
        .interpret_declarative_schema(declarative_schema);
    assert!(
        schema_result.is_ok(),
        "Schema interpretation failed: {:?}",
        schema_result
    );
...
fn test_declarative_transform_creation() {
    ...
    TestAssertions::assert_transform_output(
        &transform,
        "BlogPostWordIndex.key",
        "Transform creation test",
    );

    // Verify declarative schema is accessible
    let retrieved_schema = transform
...
fn test_declarative_transform_registration() {
    ...
    let registration_result = fixture
        .schema_core
        .register_declarative_transform(&declarative_schema);
    assert!(
        registration_result.is_ok(),
        "Transform registration failed: {:?}",
        registration_result
    );
    ...
    assert!(
        stored_transform.is_ok(),
        "Failed to retrieve stored transform"
    );
    assert!(
        stored_transform.unwrap().is_some(),
...
fn test_complex_declarative_schema() {
    ...
    assert!(
        schema_result.is_ok(),
        "Complex schema interpretation failed: {:?}",
        schema_result
    );
...
fn test_error_handling_invalid_schemas() {
    ...
    let _result = fixture
        .schema_core
        .interpret_declarative_schema(invalid_schema);
...
fn test_different_schema_types() {
    ...
    let single_result = fixture
        .schema_core
        .interpret_declarative_schema(single_schema);
    ...
    let range_result = fixture
        .schema_core
        .interpret_declarative_schema(range_schema);
...
fn test_field_mapping_registration() {
    ...
    let transform = stored_transform
        .unwrap()
        .expect("Transform should be stored");
    assert!(
        transform.is_declarative(),
        "Stored transform should be declarative"
    );
...
fn test_invalid_field_expressions() {
    ...
    for (i, invalid_expr) in invalid_expressions.iter().enumerate() {
        ...
        assert!(
            validation_result.is_err(),
            "Transform with invalid expression '{}' should fail validation",
            invalid_expr
        );
    }
}
...
fn test_missing_required_fields() {
    ...
    assert!(
        validation_result.is_err(),
        "HashRange schema without key should fail validation"
    );
    ...
    assert!(
        validation_result.is_err(),
        "Range schema with missing range_key field should fail validation"
    );
}
...
fn test_empty_and_invalid_field_definitions() {
    ...
    assert!(
        validation_result.is_ok(),
        "Schema with empty field name currently passes validation"
    );
    ...
    assert!(
        validation_result.is_ok(),
        "Field without atom_uuid but with field_type should pass validation"
    );
}
...
fn test_conflicting_field_types() {
    ...
    assert!(
        validation_result.is_ok(),
        "Range schema with Single field types currently passes validation"
    );
}
...
fn test_invalid_key_configurations() {
    ...
    assert!(
        validation_result.is_err(),
        "HashRange with empty hash_field should fail validation"
    );
    ...
    assert!(
        validation_result.is_err(),
        "HashRange with empty range_field should fail validation"
    );
    ...
    assert!(
        validation_result.is_err(),
        "HashRange with same hash and range fields should fail validation"
    );
}
...
fn test_malformed_atom_uuid_expressions() {
    ...
    for (i, malformed_expr) in malformed_expressions.iter().enumerate() {
        ...
        assert!(
            validation_result.is_err(),
            "Transform with malformed expression '{}' should fail validation",
            malformed_expr
        );
    }
}
...
fn test_invalid_schema_names() {
    ...
    for invalid_name in invalid_names {
        ...
        assert!(
            validation_result.is_err(),
            "Schema with invalid name '{}' should fail validation",
            invalid_name
        );
    }
    ...
    for allowed_name in allowed_names {
        ...
        assert!(
            validation_result.is_ok(),
            "Schema with name '{}' should pass validation",
            allowed_name
        );
    }
}
...
fn test_empty_inputs() {
    ...
    assert!(
        validation_result.is_ok(),
        "Transform with empty inputs currently passes validation"
    );
    ...
    assert!(
        validation_result.is_err(),
        "Transform with empty string input should fail validation"
    );
}
...
fn test_empty_output() {
    ...
    assert!(
        validation_result.is_err(),
        "Transform with empty output should fail validation"
    );
}
...
fn test_whitespace_only_values() {
    ...
    assert!(
        validation_result.is_err(),
        "Schema with whitespace-only name should fail validation"
    );
    ...
    assert!(
        validation_result.is_err(),
        "Transform with whitespace-only output should fail validation"
    );
}
...
fn test_extremely_long_values() {
    ...
    assert!(
        validation_result.is_err(),
        "Schema with extremely long name should fail validation"
    );
    ...
    assert!(
        validation_result.is_err(),
        "Transform with extremely long expression should fail validation"
    );

    if let Err(e) = validation_result {
        let error_msg = format!("{:?}", e);
        assert!(
            error_msg.contains("too long") || error_msg.contains("Expression too long"),
...
fn test_control_characters() {
    ...
    assert!(
        validation_result.is_err(),
        "Schema with control characters should fail validation"
    );
    ...
    assert!(
        validation_result.is_ok(),
        "Transform with control characters currently passes validation"
    );
}
