...
impl TransformRunner for MockTransformRunner {
    fn execute_transform_now(&self, transform_id: &str) -> Result<serde_json::Value, SchemaError> {
        println!("🚀 DIAGNOSTIC: MockTransformRunner executing transform: {}", transform_id);
    ...
    fn get_transforms_for_field(&self, schema_name: &str, field_name: &str) -> Result<HashSet<String>, SchemaError> {
        println!("🔍 DIAGNOSTIC: MockTransformRunner.get_transforms_for_field called for {}.{}", schema_name, field_name);
        ...
        if schema_name == "TransformBase" {
            ...
            println!("✅ DIAGNOSTIC: Returning mock transform: transform_for_{}_{}", schema_name, field_name);
            Ok(transforms)
        } else {
            println!("ℹ️ DIAGNOSTIC: No mock transforms for {}.{}", schema_name, field_name);
            Ok(HashSet::new())
        }
    }
}
...
fn test_transform_trigger_diagnostic_fix() {
    ...
    let test_tree = db.open_tree("test_event_monitor").expect("Failed to create test tree");
    ...
    match field_value_consumer.recv_timeout(Duration::from_millis(TEST_WAIT_MS)) {
        Ok(field_event) => {
            ...
            match triggered_consumer.recv_timeout(Duration::from_millis(TEST_WAIT_MS)) {
                Ok(triggered_event) => {
                    ...
                    assert_eq!(triggered_event.transform_id, "transform_for_TransformBase_input_field");
        ...
        Err(e) => {
            panic!("❌ DIAGNOSTIC FIX FAILED: FieldValueSet event not received: {}", e);
        }
    }
...
fn test_transform_trigger_with_no_transforms() {
    ...
    // Create EventMonitor
    let test_tree = db.open_tree("test_no_transforms").expect("Failed to create test tree");
    let persistence = PersistenceManager::new(test_tree);
    let _event_monitor = EventMonitor::new(
        Arc::clone(&message_bus),
        Arc::clone(&mock_transform_runner) as Arc<dyn TransformRunner>,
