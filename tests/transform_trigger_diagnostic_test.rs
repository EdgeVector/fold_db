...
fn register_single_field_schema(db_ops: &DbOperations, schema_name: &str, field_name: &str) {
...
fn test_transform_trigger_diagnostic_fix() {
    ...
    register_single_field_schema(&db_ops, "TransformBase", "input_field");
...
fn test_transform_trigger_with_no_transforms() {
    ...
    let db_ops = DbOperations::new(db.clone()).expect("Failed to create DbOperations");
    register_single_field_schema(&db_ops, "UserSchema", "username");
    let db_ops = Arc::new(db_ops);
    let message_bus = Arc::new(MessageBus::new());
