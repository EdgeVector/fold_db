...
async fn test_batch_index_merges_existing_entries() {
    ...
    assert!(!search_results.is_empty(), "Should find results for 'foo' after first batch");
    
    let foo_records: Vec<_> = search_results.iter()
        .filter(|r| r.key_value.hash.as_deref() == Some("recordA"))
        .collect();
    assert_eq!(foo_records.len(), 1, "Should find exactly 1 occurrence of recordA for 'foo'");
    ...
    for result in &search_results {
        eprintln!("  - Record: {:?}, Field: {}, Value: {}", 
            result.key_value, result.field, result.value);
    }
    
    assert!(!search_results.is_empty(), "Should find results for 'foo' after second batch");
    
    // Verify we have both recordA and recordB
    let record_a_count = search_results.iter()
        .filter(|r| r.key_value.hash.as_deref() == Some("recordA"))
        .count();
    let record_b_count = search_results.iter()
