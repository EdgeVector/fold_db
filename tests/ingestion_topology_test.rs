...
fn test_topology_inference_from_sample_data() {
    use serde_json::json;
    use datafold::schema::types::{Schema, SchemaType, TopologyNode, PrimitiveType};
    ...
    let mut schema = Schema::new(
        ...
        SchemaType::Single,
        None,
        Some(vec!["id".to_string(), "name".to_string(), "age".to_string(), "tags".to_string()]),
        None,
        None,
    );
    ...
    let sample_map: std::collections::HashMap<String, serde_json::Value> = 
        sample_data.as_object().unwrap().iter().map(|(k, v)| (k.clone(), v.clone())).collect();
