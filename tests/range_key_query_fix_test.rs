...
fn test_range_key_query_fixes() {
    ...
    let result_1 = fixture
        .query_range_key("test_data", "1")
        .expect("Failed to query for range key 1");

    println!(
        "📋 Query result for key '1': {}",
        serde_json::to_string_pretty(&result_1).unwrap()
    );

    // Verify that we get data for key "1"
    let key_1_data = result_1.get("1").expect("Should have key '1' in result");
    assert!(
        key_1_data.is_string() || key_1_data.is_object(),
        "Key '1' should have some data"
    );

    // Test Bug Fix 2: Verify simplified format
    if key_1_data.is_string() {
        assert_eq!(
            key_1_data.as_str().unwrap(),
            "a",
            "Key '1' should return simplified value 'a'"
        );
    ...
    let result_2 = fixture
        .query_range_key("test_data", "2")
        .expect("Failed to query for range key 2");

    println!(
        "📋 Query result for key '2': {}",
        serde_json::to_string_pretty(&result_2).unwrap()
    );
    ...
    if key_2_data.is_string() {
        assert_eq!(
            key_2_data.as_str().unwrap(),
            "b",
            "Key '2' should return simplified value 'b'"
        );
    ...
    assert!(
        result_1.get("2").is_none(),
        "Query for key '1' should NOT contain key '2' data"
    );
    assert!(
        result_2.get("1").is_none(),
        "Query for key '2' should NOT contain key '1' data"
    );
