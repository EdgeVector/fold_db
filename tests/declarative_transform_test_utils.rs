...
impl TestTransformBuilder {
    ...
    pub fn create_transform_registration(
        ...
        let transform =
            Transform::from_declarative_schema(schema, inputs.clone(), output.to_string());
...
impl TestAssertions {
    ...
    pub fn assert_schema_validation_success(
        result: Result<(), datafold::schema::types::SchemaError>,
        context: &str,
    ) {
        assert!(
            result.is_ok(),
    ...
    pub fn assert_schema_validation_failure(
        result: Result<(), datafold::schema::types::SchemaError>,
        context: &str,
    ) {
        assert!(
    ...
    pub fn assert_transform_is_declarative(transform: &Transform, context: &str) {
        assert!(
            ...
            "{}: Transform should be declarative",
            context
        );
        assert!(
            !transform.is_procedural(),
            "{}: Transform should not be procedural",
            context
        );
    }
    ...
    pub fn assert_transform_is_procedural(transform: &Transform, context: &str) {
        assert!(
            transform.is_procedural(),
            "{}: Transform should be procedural",
            context
        );
        assert!(
            !transform.is_declarative(),
            "{}: Transform should not be declarative",
            context
        );
    }
