...
async fn test_blogpost_wordindex_transform_registration() {
    ...
    for field in &expected_trigger_fields {
        let transforms_for_field = transform_manager
            .get_transforms_for_field("BlogPost", field.split('.').nth(1).unwrap())
            .expect("Failed to get transforms for field");

        println!(
            "🔗 Transforms for field '{}': {:?}",
            field, transforms_for_field
        );
    ...
    assert!(
        transform_manager
            .list_transforms()
            .expect("Failed to list transforms")
            .contains_key(expected_transform_id),
    ...
    for (transform_id, transform) in &registered_transforms {
        ...
        assert!(
            schema.key.is_some(),
            "Transform '{}' should have key configuration",
            transform_id
        );
...
async fn test_blogpost_wordindex_from_file() {
    ...
    println!(
        "📋 Transforms registered from file: {:?}",
        registered_transforms.keys().collect::<Vec<_>>()
    );
