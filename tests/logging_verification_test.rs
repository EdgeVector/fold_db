...
async fn test_lambda_logging_integration() {
    ...
    let user_logger =
        LambdaContext::get_user_logger("test_user_123").expect("Failed to get user logger");
    user_logger
        .info("user_event", "User specific log")
        .await
    ...
    assert!(
        has_system_log,
        "The mock logger should have captured the system log entry"
    );
    assert!(
        has_user_log,
        "The mock logger should have captured the user log entry"
    );
    assert!(
        has_implicit_log,
        "The mock logger should have captured the implicit log entry"
    );
